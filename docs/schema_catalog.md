...
## Purpose
一次DB (PostgreSQL) の **43 物理テーブル** (Phase 6 baseline 42 + M20 で追加された `dashboard_top_candidates` 1 表) + ローカルアーティファクト **2 ファイル**について、論理名 / 物理名 / 目的 / 主キー / 外部キー / 粒度 / 更新方針 / 保持クラス / 正本 / 下流消費者 を**契約として固定**する。Alembic 初期 migration、Repository 設計、分析クエリ、retention ジョブの単一ソース。

## Scope
- Phase 6 baseline 42 表 + Iter2 M20 で追加された 1 表 = **43 DB テーブル** と **2 ローカル永続ファイル**
...
| チェック項目 | 観点 |
|---|---|
| 表数 | Alembic で作成される表 = 43 (Phase 6 baseline 42 + M20 `dashboard_top_candidates` 1; 本書 43 + D2 で追加されうる backtest_* 系は別枠) |
| グループ別の数 | A=4 / B=3 / C=4 / D=7 / E=4 / F=2 / G=9 / H=4 / I=6 = 43 (H は Phase 6 baseline 3 + M20 追加 1) |
...
| Common Keys 列 | 43 表全てに Phase 3 / 6.10 / 6.18 の Common Keys が漏れなく列定義されているか |
