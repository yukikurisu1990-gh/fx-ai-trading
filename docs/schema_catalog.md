...
## Purpose
一次DB (PostgreSQL) の **42 物理テーブル** + ローカルアーティファクト **2 ファイル**について、論理名 / 物理名 / 目的 / 主キー / 外部キー / 粒度 / 更新方針 / 保持クラス / 正本 / 下流消費者 を**契約として固定**する。Alembic 初期 migration、Repository 設計、分析クエリ、retention ジョブの単一ソース。

## Scope
- Phase 6 時点で確定した **42 DB テーブル** と **2 ローカル永続ファイル**
...
- `docs/phase6_hardening.md` 6.20 (Phase 6 時点 canonical 42 テーブル)
...
| 41 | `supervisor_events` | Supervisor / 起動検査 / safe_stop / config_version 計算 / account_type 検証 等のイベント | `supervisor_event_id` | — | 1 Supervisor イベント | AO | Supervisor | Supervisor/Reconciler (永続) |
...
| チェック項目 | 観点 |
|---|---|
| 表数 | Alembic で作成される表 = 42 (本書 42 + D2 で追加されうる backtest_* 系は別枠) |
| グループ別の数 | A=4 / B=3 / C=4 / D=7 / E=4 / F=2 / G=9 / H=3 / I=6 = 42 |
...
| Common Keys 列 | 42 表全てに Phase 3 / 6.10 / 6.18 の Common Keys が漏れなく列定義されているか |
