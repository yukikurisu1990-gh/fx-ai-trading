...
**採用規則の既知の性質（開示）**: 対称な厚い裾（t3 まで）は罰しない。**日次 P&L の歪度が +2 程度の
book は tail 依存として kill する**（その利益は 3σ を超える日に実際にある）— FX の日次 portfolio P&L で
現実的な歪度（|skew| ≲ 1）では発火率 ≤ 0.03。4σ 程度の小さな外れ日が 40 日以上ある book は、
Sharpe 0.5 以上だと一部見逃す。scale（MAD）が測れない（定数系列など）場合は kill 側に倒す。
