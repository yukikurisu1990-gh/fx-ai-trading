...
3. **両側 1% を winsorize した日次 net の平均 ≤ 0**（利益が極端な日にしかない）
...
合成 iid 850 日、各 20,000 回（`tail_kill_mc`）での発火率:

| 日次 net の分布 | 旧 kill（上位 5 日除外）SR 0.3–0.4 | 新 kill（1% winsorize）SR 0.3–0.4 | 新 kill SR 0.5–0.8 |
| --- | --- | --- | --- |
| 正規 | 0.003–0.004 | 0.000 | 0.000 |
| t6 | 0.43–0.46 | 0.000 | 0.000 |
| Laplace | 0.56–0.60 | 0.000 | 0.000 |
| t4 | 0.73–0.77 | 0.003–0.004 | 0.000 |
| **lottery**（稀な大きな上昇日だけで稼ぐ） | 0.999–1.000 | **0.97–0.99** | **0.83–0.89** |

新 kill は対称な厚い裾を罰せず、「少数の外れ日にしか利益がない」形だけを落とす。上位 5 日を除いた
net と top 1/5/10 日占有率は報告する。
