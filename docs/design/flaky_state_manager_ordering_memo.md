# Design Memo — StateManager 順序決定性 要調査 (Cycle 6.12)

> **ステータス**: 要調査 (investigation placeholder). 本メモは結論ではなく、後続セッションで人間監督下に実施する F-2 調査への入口である。
...
1. **再現テスト PR** (read-only, 新規テスト追加のみ) — 同一ミリ秒を強制する fixture で R-1 が決定的に発火するか検証
2. 仮説確定後、**修正 PR** (1 PR = 1 責務) — 方針 A / B / C のいずれか選定
3. **本メモ更新** — 調査結果を追記、`ステータス: 調査完了` に遷移

> 本メモの更新は修正 PR と同一タイミングで実施すること。調査完了後も記録として保持する。
