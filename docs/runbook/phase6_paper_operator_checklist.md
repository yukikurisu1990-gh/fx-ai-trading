...
python -m scripts.run_paper_evaluation \
    --account-id acct-paper-eval \
    --instrument EUR_USD --direction buy --strategy minimum \
    --units 1000 --max-iterations 500 \
...
- `--max-iterations` は記録ファイルの行数より**小さく**取る。entry / broker / exit が同じ replay feed を共有するため 1 tick あたり複数行を消費する（M-3 共有 feed 設計、§10.9 と同じ）。記録ファイルが先に exhaust すると `ReplayQuoteFeed` が `ReplayExhaustedError` を投げ、loop の `except Exception` 経由で残り tick を空転して終わる（rc=0、ログにエラーが連続）。
...
- `instrument` は記録ファイルと一致させる（PR1 の慣習: instrument は filename に込める / dataset 内 line には instrument を書かない）。mismatch は warning ログを出して継続する。
