...
7. **UI は売買系を直接参照しない** — Dashboard Query Service 経由、UI 停止で売買影響なし。UI 層はさらに 4 層境界 (Secret / Runtime 接続 / Runtime mode / Operational) に従い、`ConfigProvider` / `SecretProvider` / `dashboard_query_service` / `scripts/ctl.py` の上に**薄いラッパとしてのみ存在する** (operations.md §15 / implementation_contracts.md §2.15)。secret 書込 sink は `.env` 限定、UI から DB / `app_settings` への secret 平文書込禁止、新規 ctl usecase は UI 都合では追加しない。
