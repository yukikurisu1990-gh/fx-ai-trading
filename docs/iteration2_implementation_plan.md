...
- TSS（Trade Suitability Score）計算 + `dashboard_top_candidates` mart の本生成
...
- Supervisor 1 分メトリクスループ（9 項目）
- Learning UI（enqueue / status / history）+ LearningOps 最小
- ctl CLI の実 process 管理（start / stop）
...
**総工数目安**: 30–40 日（1 人日換算）。並列化・中断考慮で実時間 6–9 週間想定。
...
- [ ] `OandaBroker._verify_account_type_or_raise` が demo/live 両方で正しく動作
...
**入力**:
- Iter1 M8 Reconciler 骨格、6.12 Action Matrix、6.2 MidRun
...
**完了条件**:
- [ ] Reconciler Action Matrix の 11 ケース全てに explicit test（Iteration 1 時点で 27 test だが分岐網羅の再確認）
...
- [ ] safe_stop 中の in-flight order 処理は既存 test を壊さない
...
**入力**:
- Iter1 M7 Supervisor、D4 6.2 メトリクス 9 項目仕様
- Carryover: **M-METRIC-1**

**変更対象**:
- `src/fx_ai_trading/supervisor/metrics_loop.py`（新規、60s 周期の記録ループ）
- `src/fx_ai_trading/supervisor/supervisor.py`（metrics_loop の起動組込）
...
- [ ] 9 項目（D4 6.2）が supervisor_events に 1 行として記録
- [ ] メトリクスは Common Keys + cycle_id で join 可能
- [ ] safe_stop 時に MetricsLoop も停止（fire sequence を破らない）
...
**リスク**:
- DB 負荷 → 9 項目 + 1 分毎なら年間 ~525k 行、許容範囲
...
**入力**:
- Iter1 M6 Notifier dispatcher、6.13 二経路契約
- Iter1§13
...
**変更対象**:
- `alembic/versions/0011_app_settings_phase_mode_seed.py`（新規 migration、key 追加）
...
- `tests/unit/test_dashboard_query_service_extended.py`

**完了条件**:
- [ ] `phase_mode` / `environment` が app_settings seed に存在
- [ ] dashboard_query_service に 4 つの新規 query（fallback 不要、実 DB 返却）
- [ ] 既存 query（get_open_positions / get_recent_orders / ...）を破壊しない
...
- [ ] `dashboard_top_candidates` テーブルに top N 行が INSERT（UPSERT）**既存 schema 利用、新規テーブル 0**
...
- [ ] Phase 7 の本 LearningOps を差し込める Interface（`LearningExecutor` Protocol）を宣言
...
**変更対象**:
- `scripts/ctl.py`（start: Supervisor process 起動、stop: graceful shutdown、emergency-flat-all: 2-factor）
...
- [ ] 2-factor は Interface として定義され、Phase 8 の SSO 版に差し替え可能
...
**入力**:
- Iter1 M5 Repository、Iter1§13、D3 ProjectionTransport Protocol
...
**完了条件**:
- [ ] Projector Interface（`ProjectionTransport` Protocol）が定義済
...
- `src/fx_ai_trading/ops/service_mode.py`（新規、multi_service_mode Interface 拡張、接続点定義）
...
- [ ] 禁止 lint が Iteration 1 時点から 2 項目以上追加: **(a)** live API key の secret 外漏れ検出、**(b)** 2-factor bypass パターン検出
...
| 2 | alembic 46 tables | M24 保持 | Full | Full 維持 |
...
**MVP #2（alembic tables 数）の明示**: Iter1 時点の **46 tables を維持**。Iter2 で新規テーブルは追加しない（`dashboard_top_candidates` mart は Iter1 で既に schema に存在、Iter2 は中身のみ生成）。app_settings の **seed 追加**（phase_mode / environment）のみで、テーブル数は不変。
...
- multi_service_mode / container_ready_mode の本運用（Iteration 2 は Interface 拡張のみ）
...
| M18 | unit | `tests/unit/test_dashboard_query_service_extended.py` |
...
| integration | 14 本（新規）+ 既存拡張 1 本 = 15 本 |
| migration | 2 本 |
| **総計** | **29 本（新規）+ 2 本（既存拡張）** |
...
1. **13 マイルストーン M13–M25** の目的・入力・完了条件・テスト条件・リスク・後続依存
2. **Carryover Mapping**: 8 Carryover ID 全てが特定 M に割り当て済
3. **MVP §12 の 10 項目全て Full 化** を Iteration 2 完了条件
4. **Phase 7 / Phase 8 送り項目** を明示
5. **改善ループによる自己精査** 済（本書は最低 Loop 2 を経由）
