...
- **`EmailNotifier`** — SMTP or SES、**任意 (MVP では未実装可)**
...
| `config.version_changed_production` | info | config_version 変更 |
...
- 以下のイベントは `notification_outbox` を**経由しない**:
  - `safe_stop.fired`
  - `safe_stop.cleared`
  - `db.critical_write_failed`
  - `db.connection_lost_sustained`
  - `stream.gap_sustained`
  - `reconciler.mismatch_manual_required`
  - `ntp.skew_reject`
  - その他 severity=critical かつ DB 非依存性を要求するイベント
- これらは `FileNotifier` への fsync + **利用可能な全外部 Notifier (`SlackNotifier` 等) への同期直接送信**を行う
...
- 環境別に通知先を変える (local: file のみ、vps/aws: file + slack 必須、email は任意)
...
**Emergency Flat CLI**:
- OS レベル CLI (`ctl emergency-flat-all`) を提供
...
**Paper → Live の誤爆防止の 4 重防御**:
1. `local` プロファイルでは live 設定不可 (設定レイヤの防御)
2. 起動時 assertion (起動レイヤの防御)
...
- [ ] **6.13 EmailNotifier** — 任意 (MVP では未実装可)
