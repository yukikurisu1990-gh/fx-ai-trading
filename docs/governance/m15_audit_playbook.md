...
| **Exploratory Strategy Research, Rounds 1 and 2** (2026-09-05) | `NON_DECISION_BEARING_EXPLORATORY_ONLY`, recorded here only because the next row turns on them. Round 1 found no edge at the M15 decision scale and left one candidate standing, a 4-to-6 day reversal; Round 2 pre-registered a 39-configuration family and classified it `MULTI_DAY_REVERSAL_UNRESOLVED_INSUFFICIENT_DETECTION_POWER` — present in the sample, not separable from zero, needing about 3.67x the span. Records: `docs/research/m15_track_a_exploratory_round_{1,2}_*.md` |
| **Supplemental historical read** (2026-09-05) | ✅ **run once** under an explicit human + ChatGPT authorisation, span resolved from the archive manifest and committed at `ae18734` **before** any content was read. `2023-04-26 … 2025-04-24` (730 UTC dates) x `PAIRS_20` x `M1` -> 14,745,861 M1 rows -> 993,878 M15 bars. Operations `track_a_supplemental_historical_read` / `track_a_supplemental_m15_derivation`, scope `SUPPLEMENTAL_EXPLORATORY_HISTORY`. **That span is now `EXPLORATORY_SEEN_DATA` and cannot be `UNSEEN` again** — it is not available as a future holdout, and neither is the development corpus. Read through `scripts/research/exploratory_m15/supplemental.py`, a separate route: `bars._assert_span` was **not** weakened, widened or parameterised, and nothing on the R1 fingerprint surface (`e147542a…`) was touched — an audit re-derived surface 32 / `e147542a…` rather than taking the claim — so the two grants in force are unaffected. The historical `EXPLORATORY_OOS_SLICE`, the dead window and the forward epoch were **not** read, verified against the caches by both audit roles. ⚠ Two audits found that a **malformed** span bound (`end="2025"`) defeated both guards and would have decoded OOS rows; no such read occurred, and both guards now refuse any bound that is not an exact `YYYY-MM-DD`. The authorisation is recorded at §0 of the results document |
| **Supplemental replication verdict** (2026-09-05) | `MULTI_DAY_REVERSAL_FAILED_SUPPLEMENTAL_HISTORY_REPLICATION`. The Round 2 candidate, frozen, returns −577.3 pips/pair net and **−426.5 gross** over the supplemental span against +262.1 net / +324.1 gross on the development window; 1 of 20 pairs positive, 0 of 9 neighbourhood cells positive on gross or net, mean IC +2.35% against −25.07%. The two periods' per-day rates differ by −2.16 pips/pair/day, CI95 [−3.68, −0.70], two-sided `p = 0.0042`. `TRACK_A_SUPPLEMENTAL_HISTORICAL_REPLICATION_COMPLETED`. Exploratory throughout: not decision-bearing, not holdout evidence, and **not** authority to invert the rule or start a new search. Record: `docs/research/m15_track_a_supplemental_replication_results.md` |
| **Reversal family closed** (2026-09-06) | `MULTI_DAY_REVERSAL_FAMILY_DROPPED_FROM_ACTIVE_EXPLORATORY_RESEARCH`, recorded beside the standing `MULTI_DAY_REVERSAL_FAILED_SUPPLEMENTAL_HISTORY_REPLICATION`. A resource decision, **not** a claim that no multi-day reversal exists: further optimisation of this family is not worth the effort. Record: `docs/research/m15_track_a_momentum_hypothesis_plan.md` |
| **Momentum fresh-history read** (2026-09-06) | ✅ **run once** under an explicit human + ChatGPT authorisation. `MOMENTUM_SUPPLEMENTAL_REPLICATION_B` = `2021-04-26 … 2023-04-25` (730 UTC dates) x `PAIRS_20` x `M1` -> 14,591,042 M1 rows -> 999,238 M15 bars. Span resolved from the archive manifest and committed at `c0ba1cd` **before** any content was read; operations `track_a_momentum_historical_read` / `track_a_momentum_m15_derivation`, scope `MOMENTUM_SUPPLEMENTAL_EXPLORATORY_HISTORY`. **That span is now `EXPLORATORY_SEEN_DATA` and cannot be `UNSEEN` again.** Read through `scripts/research/exploratory_m15/momentum.py`, a **third** route: `bars._assert_span` and `supplemental.assert_supplemental_span` are byte-identical, the new guard's upper bound **is** `supplemental.SUPPLEMENTAL_START_UTC`, and a test asserts each of the three windows is admitted by exactly one guard while the OOS slice, the dead window and the forward epoch are admitted by none. Nothing on the R1 fingerprint surface (`e147542a…`) touched — re-derived to 32 files by an audit rather than asserted. ⚠ An audit found a **bypass present at base master and inherited by all three routes**: each validated its bounds' *shape* with `utc_date` and then compared **the caller's object**, so a `str` subclass overriding `__lt__`/`__ge__` was admitted and the reader decoded the whole ten-year archive. Now closed in all three by comparing the parsed `date`; behaviour on genuine strings is unchanged. No forbidden row was read — every parquet in all three caches was measured. The authorisation for the read is recorded at §0 of the results document |
| **Momentum verdict** (2026-09-06) | `MULTI_DAY_MOMENTUM_UNRESOLVED_IN_FRESH_EXPLORATORY_HISTORY`. The frozen mirror candidate returns −451.4 net and −299.0 gross on the fresh span, but **nothing on that span is separable from zero**: net `p = 0.187` with CI95 `[−1128, +217]`, mean IC −5.85% **inside** its permutation band (`p = 0.233`, bias −1.48%), breadth 2/20 at `p = 0.053`, and the **reversal direction on the same 730 days is +146.7 at `p = 0.667`**. Power against the alternative that motivated the hypothesis was **0.124**, where the previous round earned a *failed-replication* verdict at ~0.76. ⚠ **The first draft said FAILED**; two review roles showed its evidence was three statistics that carry no information — "IC positive in 1 of 8 blocks" (null expectation 1.06/8), "0 of 9 / 0 of 20 / 0 of 8" (arithmetically forced by the pooled estimate) and "the sign alternates by period" (the only new contrast is `p = 0.104`) — and that a null construction of the implementing session had understated its own spread. ⭐ **Both directions null on one span** is what the round actually established, and it is why the reversal family stays closed. B is **not** licence to continue: three consecutive spans have produced nothing in either direction. Exploratory throughout; not decision-bearing, not holdout evidence. Record: `docs/research/m15_track_a_momentum_hypothesis_results.md`
| **Next stage** (as recorded 2026-09-06; **superseded on 2026-09-14** by the pause row below) | `TRACK_A_READY_TO_BEGIN_EXPLORATORY_STRATEGY_RESEARCH`, unchanged. Feature / model / label / calibration / threshold exploration may continue over the seen historical data — which is now the development corpus, the supplemental span **and** `2021-04-26 … 2023-04-25`. Every output is `NON_DECISION_BEARING_EXPLORATORY_ONLY`. **Not** authority for R2, an OOS read, or Formal Confirmation. The four exploratory rounds above closed both directions of the only family they raised and started nothing; a next round is a new instruction, not a continuation |
| **FX spot active-alpha research paused** (2026-09-14) | `FX_SPOT_ACTIVE_ALPHA_RESEARCH_PAUSED`. The Human + ChatGPT decision of 2026-09-14 accepted Track 1's Case C (`CONTINUOUS_CURRENCY_PORTFOLIO_ARCHITECTURE_NOT_SUPPORTED_IN_SEEN_DEVELOPMENT`, PRs #481/#482) and paused active-alpha research on FX spot. **This overrides the Next stage row above**: feature / model / label / calibration / threshold exploration does **not** continue, and no new alpha hypothesis, pre-registration or alpha-seeking run starts, until an explicit Human + ChatGPT decision resumes it. Not `FX_HAS_NO_EDGE`. Record, scope and resumption conditions: `docs/research/m15_fx_spot_active_alpha_research_pause.md` |
