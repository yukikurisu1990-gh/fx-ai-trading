...
- **必要 IC との距離**: 必要 4.44%（net 0.3、半減期 5 日）に対し、観測は 1.43%（A）/ 2.36%（C）。
...
| C が B に対して正の net 増分を保つ | 満たさない（−0.512、B より高いが負） |
...
- **事前登録した設計は、この seen span では経済的に成立しない。** 理由は cost で、それは推定ではなく実測（turnover × 課金規約）。
- 速い repricing measure（半減期 1.7 日）を band 0.10 の通貨 book で運ぶと、必要 IC 4.4% に対して観測 1.4〜2.4% では届かない。
- FX momentum 単独（B）も負で、**価格情報だけの family をもう一度確認した**ことにはなる。
