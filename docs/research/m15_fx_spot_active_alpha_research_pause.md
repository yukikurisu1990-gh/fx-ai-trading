...
- **対象は通貨レベルの reversal family 全体**（5・20・60 日のどれでも、どの符号の組み合わせでも、他の horizon でも）。
  Track 1 の結果記録の停止対象と同じ範囲である。
...
- **この降格は、tail 条項で閉じた family（H-021 COT、H-022 Track A など）を再び開かない。**
...
停止中は追求しない。OPEN であることは再開の根拠にならない（RC-1）。H-015 の volatility feature は §5 の volatility filter
による救済と同じ中身で、その用途には使えない。
...
| ID | 条件 | 出典 |
| --- | --- | --- |
| RC-1 | 再開は Human + ChatGPT の明示の決定によってのみ行う。仮説・データ・操作・承認 head を名指しする。記録された status・gate・文書・この表の条件がすべて揃ったことはそれに代わらない | 既存規則 |
...
| RC-5 | 仮説は ledger の閉鎖 family と隣接 family の外にあり、novelty boundary を結果を見る前に書く。post-hoc 観察は昇格しない。隣接 = 同じ単位（通貨または pair）で、過去リターンに基づく同じ信号をhorizon・符号・閾値だけ変えたもの | 起草 |
...
| RC-8 | tail concentration は hard kill ではなく adversarial diagnostic として報告する。この降格は、tail 条項で閉じた family（H-021、H-022 など）を再び開かない | 裁定（推奨） |

RC-1 の根拠（既存規則）: CLAUDE.md の「A real-data read is Red … an act, not a document state」と「a recorded state is not an act」、
および自律開発方針の Red 承認規則。active-alpha research の再開は実データの読み取りを伴うため、既にこの規則の下にある。

## 13. Human + ChatGPT の判断事項

1. 本 PR の merge 承認。
2. 出典が `起草` と `裁定から導出` の項目の確定（§5・§9・§11 の該当行、§12 の RC-2〜RC-7）。**裁定本文が §9 で途切れているため。**
3. 「temporal co…」の語の確定。
4. 未 merge の #473（expectation benchmark）の扱いと、それが閉じる survey-consensus family を ledger に入れるか。
5. **Track 1 事前登録が開示した tail kill の見逃し（高 Sharpe の lottery 型 book）を受け入れるか。** §8 の降格はこれを解消しない。
6. 最小研究 gate §8.11.2(1)（Track A の結果を決定の証拠にしない）と、本文書 §3 が停止の理由として Track A の数値を記録することの関係。
7. OPEN のまま停止した ledger 4 件（H-011、H-015、H-018、H-019）を、停止に合わせて閉じるか。本記録は status 文を変えていない。
