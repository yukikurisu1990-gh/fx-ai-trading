...
def test_a_row_inside_the_dead_window_is_refused_even_if_declared(
    sandbox: Path, guards: object
) -> None:
    """`no_overlap` checks declarations; this checks the bytes."""
    _write_source(
        sandbox,
        "EUR_USD",
    ...
    with pytest.raises(read_route.ReadRouteError, match="dead window"):
        read_route.read_historical(
