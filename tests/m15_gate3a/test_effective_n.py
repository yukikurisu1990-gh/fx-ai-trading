...
def test_below_raw_floor_insufficient_sample() -> None:
    r = effective_n([_pp("EUR_USD", 900, 0.0)], cross_pair_corr=0.0)
...
def test_holdout_floors_are_value_pinned() -> None:
    ...
    at_floor = effective_n([_pp(PAIRS_20[i], 400, 0.0) for i in range(4)], cross_pair_corr=1.0)
...
def test_raw_floor_boundary_is_pinned() -> None:
    just_below = effective_n([_pp("EUR_USD", RAW_FLOOR - 1, 0.0)], cross_pair_corr=0.0)
    exactly_at = effective_n([_pp("EUR_USD", RAW_FLOOR, 0.0)], cross_pair_corr=0.0)
...
def test_invalid_adjustments_fail_closed() -> None:
    with pytest.raises(EffectiveNError):
        effective_n([_pp("EUR_USD", 1000, 1.5)], cross_pair_corr=0.0)
    with pytest.raises(EffectiveNError):
        effective_n([_pp("EUR_USD", 1000, 0.0)], cross_pair_corr=-0.1)
    with pytest.raises(EffectiveNError):
        effective_n([_pp("EUR_USD", -5, 0.0)], cross_pair_corr=0.0)
    with pytest.raises(EffectiveNError):
        effective_n([], cross_pair_corr=0.0)
    with pytest.raises(EffectiveNError):
        effective_n([_pp("EUR_USD", 1000, float("nan"))], cross_pair_corr=0.0)
    with pytest.raises(EffectiveNError):
        effective_n([_pp("EUR_USD", 1000, 0.0)], cross_pair_corr=float("nan"))
    with pytest.raises(EffectiveNError):
        effective_n([_pp("EUR_USD", True, 0.0)], cross_pair_corr=0.0)  # bool is not a count
    with pytest.raises(EffectiveNError):
        effective_n(
            [_pp("EUR_USD", 1, 0.0), _pp("EUR_USD", 1, 0.0)], cross_pair_corr=0.0
        )  # duplicate pair
