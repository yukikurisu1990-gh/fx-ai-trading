...
**R-2 / §12.20 — every call declares which quantity it is passing.**
``effective_n`` takes a mandatory ``count_quantity`` literal and admits only the
traded-event quantity. The confusable ``complete_bucket_count`` and
``cost_hurdle_eligible_bar_count`` are strictly larger quantities whose names the
committed inventory and the approved spec use interchangeably; feeding either one
here clears the frozen floors (raw >= 1000, N_eff >= 400) by orders of magnitude
and silently disarms ``INSUFFICIENT_SAMPLE``. The constant is imported rather than
spelled out, so a rename cannot leave these calls quietly declaring the old name.
"""
...
def test_raw_count_preserved_and_independent_recovers_raw() -> None:
    pairs = [_pp(PAIRS_20[i], 250, 0.0) for i in range(20)]
    r = effective_n(pairs, count_quantity=RAW_TRADED_EVENT_COUNT, cross_pair_corr=0.0)
...
def test_per_pair_formula_matches_approved_spec() -> None:
    ...
    pairs = [_pp("EUR_USD", 250, 0.5), _pp("GBP_USD", 250, 0.2)]
    r = effective_n(pairs, count_quantity=RAW_TRADED_EVENT_COUNT, cross_pair_corr=0.1)
...
def test_below_raw_floor_insufficient_sample() -> None:
    r = effective_n(
        [_pp("EUR_USD", 900, 0.0)], count_quantity=RAW_TRADED_EVENT_COUNT, cross_pair_corr=0.0
    )
...
def test_holdout_floors_are_value_pinned() -> None:
    ...
    pairs = [_pp(PAIRS_20[i], 250, 0.0) for i in range(4)]
    r = effective_n(pairs, count_quantity=RAW_TRADED_EVENT_COUNT, cross_pair_corr=1.0)
    ...
    at_floor = effective_n(
        [_pp(PAIRS_20[i], 400, 0.0) for i in range(4)],
        count_quantity=RAW_TRADED_EVENT_COUNT,
        cross_pair_corr=1.0,
    )
...
def test_raw_floor_boundary_is_pinned() -> None:
    just_below = effective_n(
        [_pp("EUR_USD", RAW_FLOOR - 1, 0.0)],
        count_quantity=RAW_TRADED_EVENT_COUNT,
        cross_pair_corr=0.0,
    )
    exactly_at = effective_n(
        [_pp("EUR_USD", RAW_FLOOR, 0.0)], count_quantity=RAW_TRADED_EVENT_COUNT, cross_pair_corr=0.0
    )
...
def test_invalid_adjustments_fail_closed() -> None:
    with pytest.raises(EffectiveNError):
        effective_n(
            [_pp("EUR_USD", 1000, 1.5)], count_quantity=RAW_TRADED_EVENT_COUNT, cross_pair_corr=0.0
        )
    with pytest.raises(EffectiveNError):
        effective_n(
            [_pp("EUR_USD", 1000, 0.0)], count_quantity=RAW_TRADED_EVENT_COUNT, cross_pair_corr=-0.1
        )
    with pytest.raises(EffectiveNError):
        effective_n(
            [_pp("EUR_USD", -5, 0.0)], count_quantity=RAW_TRADED_EVENT_COUNT, cross_pair_corr=0.0
        )
    with pytest.raises(EffectiveNError):
        effective_n([], count_quantity=RAW_TRADED_EVENT_COUNT, cross_pair_corr=0.0)
    with pytest.raises(EffectiveNError):
        effective_n(
            [_pp("EUR_USD", 1000, float("nan"))],
            count_quantity=RAW_TRADED_EVENT_COUNT,
            cross_pair_corr=0.0,
        )
    with pytest.raises(EffectiveNError):
        effective_n(
            ...
            count_quantity=RAW_TRADED_EVENT_COUNT,
            cross_pair_corr=float("nan"),
        )
    with pytest.raises(EffectiveNError):
        effective_n(
            [_pp("EUR_USD", True, 0.0)], count_quantity=RAW_TRADED_EVENT_COUNT, cross_pair_corr=0.0
        )  # bool is not a count
    with pytest.raises(EffectiveNError):
        effective_n(
            [_pp("EUR_USD", 1, 0.0), _pp("EUR_USD", 1, 0.0)],
            count_quantity=RAW_TRADED_EVENT_COUNT,
            cross_pair_corr=0.0,
        )  # duplicate pair
