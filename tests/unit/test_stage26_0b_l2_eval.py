"""Unit tests for Stage 26.0b-β L-2 generic path-quality regression eval.

Covers ~28 tests per 26.0b-α §10:
- Tests 1-24 adapted from 26.0a-β rev1 (L-3 → L-2 semantics).
- 3 NEW L-2-specific tests: #4 (no spread subtraction), #5 (spread
  perturbation invariance), #26 (CONCENTRATION_HIGH is diagnostic-only).
- 1 MODIFIED test: #13 (sweep grid 12 not 24).
- Plus 3 invariant guards (barrier outcome, Ridge no random_state,
  A0-prefilter).
"""
...
l2 = importlib.import_module("stage26_0b_l2_eval")
...
# Test 1
def test_l2_label_construction_deterministic():
    df = _make_row(time_to_fav_bar=20, time_to_adv_bar=-1)
    y1 = l2.build_l2_labels_for_dataframe(df, _no_runtime_map(), "raw_pip")
    y2 = l2.build_l2_labels_for_dataframe(df, _no_runtime_map(), "raw_pip")
    assert y1.iloc[0] == y2.iloc[0]


# Test 2
def test_l2_base_pnl_is_mid_to_mid_long():
    df = _make_row(
        direction="long",
    ...
    y = l2.build_l2_labels_for_dataframe(df, _no_runtime_map(), "raw_pip")
    assert y.iloc[0] == pytest.approx(l2.K_FAV * 10.0, abs=1e-9)


# Test 3
def test_l2_base_pnl_is_mid_to_mid_short():
    df = _make_row(
        direction="short",
        atr_at_signal_pip=10.0,
        spread_at_signal_pip=0.0,
...
# Test 4 (L-2 NEW)
def test_l2_no_spread_subtraction_in_label():
    """L-2 §3.1 binding: label = base_mid_pnl (no D-4 spread subtraction).

    For TP outcome at K_FAV=1.5, atr=10, spread=2 → L-2 label = 1.5*10 = 15.0
    (L-3 would give 1.5*10 - 2 = 13.0). Test enforces the L-2 = 15.0 value.
    """
    df = _make_row(time_to_fav_bar=20, atr_at_signal_pip=10.0, spread_at_signal_pip=2.0)
    y = l2.build_l2_labels_for_dataframe(df, _no_runtime_map(), "raw_pip").iloc[0]
    # L-2: label = K_FAV * atr = 15.0 (NO spread subtraction)
    assert y == pytest.approx(l2.K_FAV * 10.0, abs=1e-9)
    # Equivalently: spread value MUST NOT affect label.
    df0 = _make_row(time_to_fav_bar=20, atr_at_signal_pip=10.0, spread_at_signal_pip=0.0)
    y0 = l2.build_l2_labels_for_dataframe(df0, _no_runtime_map(), "raw_pip").iloc[0]
    assert y == pytest.approx(y0, abs=1e-12)


# Test 5 (L-2 NEW)
def test_l2_target_unchanged_when_spread_perturbed():
    """Perturbing spread_at_signal_pip from 0 to 100 — L-2 label is IDENTICAL."""
    df_zero = _make_row(spread_at_signal_pip=0.0, time_to_fav_bar=20, atr_at_signal_pip=10.0)
    df_hundred = _make_row(spread_at_signal_pip=100.0, time_to_fav_bar=20, atr_at_signal_pip=10.0)
    y_zero = l2.build_l2_labels_for_dataframe(df_zero, _no_runtime_map(), "raw_pip").iloc[0]
    y_hundred = l2.build_l2_labels_for_dataframe(df_hundred, _no_runtime_map(), "raw_pip").iloc[0]
    assert y_zero == pytest.approx(y_hundred, abs=1e-12), (
        "L-2 label must be invariant under spread_at_signal_pip perturbation"
    )


# Test 5/6 (L-2 single-axis change confirmation; no D-4 spread-factor knob)
def test_l2_has_no_spread_factor_parameter():
    """L-2 build_l2_labels_for_dataframe must NOT take a spread_factor arg."""
    import inspect

    sig = inspect.signature(l2.build_l2_labels_for_dataframe)
    param_names = set(sig.parameters.keys())
    assert "spread_factor" not in param_names, (
        "L-2 must not accept spread_factor (D-4 omitted per 26.0b-α §3.1)"
    )
    # Must accept (df, pair_runtime_map, target_scale) only
    assert param_names == {"df", "pair_runtime_map", "target_scale"}


def test_l2_module_has_no_spread_factors_constant():
    """L-2 module must NOT define SPREAD_FACTORS (D-4 omitted)."""
    assert not hasattr(l2, "SPREAD_FACTORS"), (
        "L-2 must NOT define SPREAD_FACTORS (D-4 omitted per 26.0b-α §3.1)"
    )


# Test 7
def test_l2_inherits_triple_barrier_k_fav_k_adv():
    assert pytest.approx(1.5) == l2.K_FAV
    assert pytest.approx(1.0) == l2.K_ADV


# Test 8
def test_l2_horizon_expiry_uses_m5_close_mark_to_market():
    n_m1 = 200
    ...
    entry_idx = 31
    exit_idx = entry_idx + l2.H_M1_BARS - 1
    ...
    y = l2.build_l2_labels_for_dataframe(df, pair_runtime_map, "raw_pip").iloc[0]
    # L-2: no spread subtraction; base mid-PnL is 10 pip (mid_exit - mid_entry).
    expected = 10.0
    assert y == pytest.approx(expected, abs=1e-6)


# Test 9
def test_l2_label_excludes_mfe_mae():
    df_base = _make_row(
        time_to_fav_bar=20,
        time_to_adv_bar=-1,
        atr_at_signal_pip=10.0,
    ...
    y_a = l2.build_l2_labels_for_dataframe(df_a, _no_runtime_map(), "raw_pip").iloc[0]
    y_b = l2.build_l2_labels_for_dataframe(df_b, _no_runtime_map(), "raw_pip").iloc[0]
    assert y_a == pytest.approx(y_b, abs=1e-12)


# Test 10
def test_l2_atr_normalised_scale():
    df = _make_row(time_to_fav_bar=20, atr_at_signal_pip=10.0, spread_at_signal_pip=2.0)
    y = l2.build_l2_labels_for_dataframe(df, _no_runtime_map(), "atr_normalised").iloc[0]
    # L-2: label = K_FAV * atr / atr = K_FAV (no spread subtraction in L-2).
    expected = l2.K_FAV
    assert y == pytest.approx(expected, abs=1e-9)


# Test 11
def test_l2_raw_pip_scale():
    df = _make_row(time_to_fav_bar=20, atr_at_signal_pip=10.0, spread_at_signal_pip=2.0)
    y = l2.build_l2_labels_for_dataframe(df, _no_runtime_map(), "raw_pip").iloc[0]
    # L-2: label = K_FAV * atr (no spread subtraction).
    expected = l2.K_FAV * 10.0
    assert y == pytest.approx(expected, abs=1e-9)


# Test 12
def test_l2_winsorise_thresholds_fit_on_train_only():
    rng = np.random.default_rng(42)
    train_y = pd.Series(rng.normal(0.0, 1.0, size=2000))
    lo, hi = l2.fit_winsorise_train(train_y)
    assert -3.0 < lo < -1.5
    assert 1.5 < hi < 3.0
    lo_again, hi_again = l2.fit_winsorise_train(train_y)
...
def test_l2_winsorise_does_not_touch_realised_pnl_for_harness():
    ...
    """
    sig = inspect.signature(l2.precompute_realised_pnl_per_row)
    param_names = set(sig.parameters.keys())
    ...
    sig2 = inspect.signature(l2._compute_realised_barrier_pnl)
    ...
    out = l2.apply_winsorise(y, lo=-2.0, hi=2.0)
    assert out.tolist() == [-2.0, -2.0, 0.0, 2.0, 2.0]


# Test 14
def test_l2_winsorise_disabled_passes_through():
    y = pd.Series([-100.0, -1.0, 0.0, 1.0, 100.0])
    out = l2.apply_winsorise(y, lo=float("-inf"), hi=float("inf"))
    np.testing.assert_array_equal(out.to_numpy(), y.to_numpy())


# Test 15
def test_sweep_grid_has_12_cells():
    """26.0b-α Decision A: 12 cells (no spread / pair-set sweep knobs)."""
    cells = l2.build_cells(include_lightgbm=True)
    assert len(cells) == 12
    cells_no_lgbm = l2.build_cells(include_lightgbm=False)
    assert len(cells_no_lgbm) == 8
...
def test_lightgbm_uses_fixed_conservative_config():
    ...
    assert expected == l2.LIGHTGBM_FIXED_CONFIG


# Test 17
def test_lightgbm_defer_path_when_not_importable():
    """L-2 graceful fallback: 8 cells (12 - 4 LightGBM cells)."""
    cells = l2.build_cells(include_lightgbm=False)
    assert len(cells) == 8
...
def test_cell_and_threshold_selection_uses_validation_only():
    ...
    cells = [
        {
            "cell": {"scale": "raw_pip", "clip": "none", "model": "Ridge"},
            "val_realised_sharpe": 0.20,  # higher val Sharpe
            "val_realised_annual_pnl": 100.0,
            "val_n_trades": 200,
            "val_max_dd": 10.0,
            "test_realised_metrics": {"sharpe": 0.05},  # lower test Sharpe
            "test_regression_diag": {"spearman": 0.1},
            "test_gates": {},
            "h_state": "OK",
        },
        {
            "cell": {"scale": "atr_normalised", "clip": "none", "model": "Ridge"},
            "val_realised_sharpe": 0.10,  # lower val Sharpe
            "val_realised_annual_pnl": 50.0,
            "val_n_trades": 200,
            "val_max_dd": 8.0,
            "test_realised_metrics": {"sharpe": 0.50},  # higher test Sharpe
            "test_regression_diag": {"spearman": 0.5},
            "test_gates": {},
            "h_state": "OK",
        },
    ]
    result = l2.select_cell_validation_only(cells)
    sel = result["selected"]
    # Selection should pick the higher-val-Sharpe cell (scale=raw_pip), NOT the
    # higher-test-Sharpe cell (scale=atr_normalised)
    assert sel["cell"]["scale"] == "raw_pip", "selection must use VAL Sharpe, not TEST Sharpe"
...
def test_verdict_tree_h2_pass_alone_not_adopt():
    ...
    result = l2.assign_verdict(val_selected)
...
def test_verdict_tree_h1_meaningful_threshold_010():
    ...
    result_a = l2.assign_verdict(val_selected_a)
    ...
    result_b = l2.assign_verdict(val_selected_b)
...
def test_h3_baseline_constant_phase25_best_minus_0192():
    assert pytest.approx(-0.192) == l2.H3_REFERENCE_SHARPE
...
def test_regression_diagnostic_returns_r2_pearson_spearman():
    ...
    diag = l2.regression_diagnostics(y_true, y_pred)
    assert "r2" in diag
...
# Test 26 (L-2 NEW; CONCENTRATION_HIGH diagnostic-only)
def test_concentration_high_flag_is_diagnostic_only():
    """26.0b-α §4.1 binding: CONCENTRATION_HIGH flag is NOT consulted by
    select_cell_validation_only or assign_verdict.
    """
    import inspect

    sel_src = inspect.getsource(l2.select_cell_validation_only)
    assert "concentration_high" not in sel_src, (
        "select_cell_validation_only must NOT reference concentration_high"
    )
    assert "top_pair_share" not in sel_src, (
        "select_cell_validation_only must NOT reference top_pair_share"
    )

    verdict_src = inspect.getsource(l2.assign_verdict)
    assert "concentration_high" not in verdict_src, (
        "assign_verdict must NOT reference concentration_high"
    )
    assert "top_pair_share" not in verdict_src, "assign_verdict must NOT reference top_pair_share"

    # Sanity: the flag exists and fires correctly on a synthetic input.
    import numpy as np
    import pandas as pd

    df = pd.DataFrame({"pair": ["USD_JPY"] * 90 + ["EUR_USD"] * 10})
    traded_mask = np.ones(100, dtype=bool)
    result = l2.compute_pair_concentration(df, traded_mask)
    assert result["top_pair"] == "USD_JPY"
    assert result["top_pair_share"] == pytest.approx(0.90)
    assert result["concentration_high"] is True

    # Balanced distribution: no flag
    df_balanced = pd.DataFrame({"pair": [f"P_{i % 10}" for i in range(100)]})
    res2 = l2.compute_pair_concentration(df_balanced, traded_mask)
    assert res2["concentration_high"] is False


def test_no_diagnostic_columns_in_feature_set():
    prohibited = set(l2.PROHIBITED_DIAGNOSTIC_COLUMNS)
    feature_cols = set(l2.CATEGORICAL_COLS)
    assert prohibited.isdisjoint(feature_cols)
    assert set(l2.CATEGORICAL_COLS) == {"pair", "direction"}
...
def test_quantile_cutoff_fits_on_val_only():
    ...
    cutoff_v1 = l2.fit_quantile_cutoff_on_val(pred_val, 10)
    ...
    # Re-fit on val only
    cutoff_v2 = l2.fit_quantile_cutoff_on_val(pred_val, 10)
...
def test_quantile_cutoff_applied_to_test_uses_val_fit_value():
    ...
    results = l2.evaluate_quantile_family(
        pred_val,
        pnl_val_zero,
        pred_test,
        pnl_test_zero,
...
def test_no_full_sample_qcut_in_quantile_path():
    ...
    """
    src = inspect.getsource(l2.evaluate_quantile_family)
    ...
    sig = inspect.signature(l2.fit_quantile_cutoff_on_val)
...
def test_quantile_threshold_family_has_5_candidates():
    assert l2.THRESHOLDS_QUANTILE_PERCENTS == (5, 10, 20, 30, 40)
    assert len(l2.THRESHOLDS_QUANTILE_PERCENTS) == 5
...
def test_negative_absolute_thresholds_secondary_informational():
    ...
    """
    src = inspect.getsource(l2.select_cell_validation_only)
    ...
    assert l2.NEG_THRESHOLDS_RAW_PIP == (-5.0, -3.0, -1.0, 0.0, 1.0)
    assert l2.NEG_THRESHOLDS_ATR == (-0.5, -0.3, -0.1, 0.0, 0.1)
...
def test_selected_threshold_family_is_quantile():
    ...
    result = l2.select_cell_validation_only([cell_result_shape])
    sel = result["selected"]
...
def test_determine_barrier_outcome_branches():
    assert l2.determine_barrier_outcome(5, 5, True) == "SL"
    assert l2.determine_barrier_outcome(10, 5, False) == "SL"
    assert l2.determine_barrier_outcome(5, 10, False) == "TP"
    assert l2.determine_barrier_outcome(-1, 5, False) == "SL"
    assert l2.determine_barrier_outcome(5, -1, False) == "TP"
    assert l2.determine_barrier_outcome(-1, -1, False) == "TIME"


def test_ridge_pipeline_has_no_random_state():
    """Per user correction: Ridge constructed WITHOUT random_state."""
    pipe = l2.build_pipeline_ridge()
...
def test_select_cell_validation_only_uses_a0_prefilter():
    ...
    a0_min = l2.A0_MIN_ANNUAL_TRADES * l2.VAL_SPAN_YEARS
    cells = [
        {
            "cell": {"scale": "raw_pip", "clip": "none", "model": "Ridge"},
            "selected_q_percent": 5,
            "val_realised_sharpe": 0.50,  # higher val Sharpe but below A0
        ...
        },
        {
            "cell": {"scale": "atr_normalised", "clip": "none", "model": "Ridge"},
            "selected_q_percent": 10,
            "val_realised_sharpe": 0.20,  # lower val Sharpe but A0-eligible
            "val_realised_annual_pnl": 100.0,
    ...
    result = l2.select_cell_validation_only(cells)
    sel = result["selected"]
    assert sel["cell"]["scale"] == "atr_normalised"
