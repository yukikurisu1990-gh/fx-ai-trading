"""compare_multipair_v11_csi.py — Phase 9.16/G-2.5 currency strength features.

Successor to v9. Same 12 orthogonal features; adds 3 per-pair Currency
Strength Index (CSI) features that aggregate cross-pair returns per
currency:

  cs_base   — strength of the pair's base currency
  cs_quote  — strength of the pair's quote currency
  cs_diff   — cs_base - cs_quote (signed direction predictor)

Per-currency strength is computed by aggregating per-pair returns:
contribution = +ret if currency is the pair's base, -ret if quote.
With 20 pairs covering 8 currencies (EUR, GBP, AUD, NZD, USD, CHF,
CAD, JPY) we get robust strength signals per currency.

Why
---
Phase 9.16/G-2 (pair expansion 10 -> 20) lifted PnL only +6% — much
less than the +25-50% predicted. Diagnosis: the existing cross-pair
features (xp_ret_rank, xp_rsi_rank, xp_basket_corr) only capture
**pair-level** ranks, not **currency-level** strength. The Phase 9.3
``CurrencyStrengthIndex`` exists in production but feeds the
MetaDecider Score stage, not Layer 1's features. G-2.5 closes that
gap.

Expected impact on top of G-2:
  Trade quality up via per-currency directional bias signals
  Sharpe target: 0.16 -> 0.18-0.22
  PnL target: 8,157 -> 9,000-12,000 (additional +10-25%)

This is the v11 backtest. Same internal sweep + MaxDD + 2-pass-RH-
optional pattern as v9/v10 (RH not enabled here; can be added by
follow-on if CSI alone hits the gate).

(Original v9 docstring follows for reference.)

----

compare_multipair_v9_orthogonal.py — Phase 9.15/F-1 orthogonal features.

Successor to v5 (`compare_multipair_v5_bidask.py`). Same bid/ask-aware
labels and vectorised eval; adds 12 orthogonal features the model
currently does not see:

  Bid/ask spread (3):
    spread_now_pip        — (ask_o - bid_o) / pip_size
    spread_ma_ratio_20    — spread / 20-bar mean
    spread_zscore_50      — 50-bar z-score of spread

  Time-of-day (6):
    hour_sin / hour_cos   — cyclic UTC hour encoding
    is_asian, is_london, is_overlap — session one-hot
    day_of_week_sin       — weekly cyclic

  Volume (2):
    volume_pct_100        — 100-bar percentile rank
    volume_zscore_50      — 50-bar z-score

  Regime (1 multi-column):
    regime_trend / regime_range / regime_high_vol — ATR-percentile
                                                    based one-hot

Total feature count: 32 (v5) -> 45 (v9).

Why
---
Phase 9.13 closure surfaced the binding constraint: every Layer-1-only
risk lever (B-3 meta-labeling, C-1 Kelly, C-2 cap) re-projects
predict_proba and adds no new info. Phase 9.15 attacks that root cause
by giving the model orthogonal information it cannot see today —
notably bid/ask spread, which we use in *labels* (B-2) but never as a
*feature*.

CLI
---
    --features-bundle BUNDLE  # one of: baseline, +spread, +time,
                              #         +volume, +regime, all
                              # (default: all)

The bundle flag activates only a subset of the new features so the
closure memo can attribute Sharpe lift per feature group.

Phase 9.10 GO target: SELECTOR net Sharpe >= 0.20. Recent_hit_rate_50
(self-feedback feature) is split into PR F-2 because it requires
sequential per-fold tally.
...
def _build_cross_pair_features(
    ...
    """Phase 9.16/G-2.5: cross-pair features = (xp ranks/corr) + (CSI features).

    The original 3 features (xp_ret_rank, xp_rsi_rank, xp_basket_corr) are
    pair-level rankings. Phase 9.16/G-2.5 adds **per-currency strength**
    (CSI) — aggregated returns across all pairs containing each currency.
    The Phase 9.3 ``CurrencyStrengthIndex`` was already implemented for
    the production MetaDecider Score stage; G-2.5 routes the same idea
    to Layer 1 features so the model can directly learn 'USD strong /
    JPY weak -> USD/JPY long bias'.

    Per-pair CSI features added (3 columns):
      cs_base     = strength of the pair's base currency
      cs_quote    = strength of the pair's quote currency
      cs_diff     = cs_base - cs_quote (signed predictor of pair direction)

    All cross-pair features are computed on the ref_pair timestamp grid
    and shifted by 1 bar before merge (anti-leak).
    """
    ...
    # --- Phase 9.16/G-2.5: per-currency strength index ---
    # Identify all currencies in the universe.
    currencies: set[str] = set()
    for pair in feat_dfs:
        base, quote = pair.split("_")
        currencies.add(base)
        currencies.add(quote)

    # For each currency C, aggregate per-pair returns:
    #   contribution = +ret if C is base of pair, -ret if C is quote.
    cs_per_currency: dict[str, pd.Series] = {}
    for cur in currencies:
        contributions: list[pd.Series] = []
        for pair in feat_dfs:
            base, quote = pair.split("_")
            if cur == base:
                contributions.append(ret_df[pair])
            elif cur == quote:
                contributions.append(-ret_df[pair])
        if contributions:
            stacked = pd.concat(contributions, axis=1)
            avg = stacked.mean(axis=1)
            # Smoothed (rolling mean) so the feature reflects regime, not
            # single-bar noise. csi_window=20 -> 20-minute strength.
            cs_per_currency[cur] = avg.rolling(csi_window, min_periods=5).mean()
    ...
    for pair, df in feat_dfs.items():
        ...
        base, quote = pair.split("_")
        cs_base = cs_per_currency.get(base, pd.Series(0.0, index=ref_ts_series.index))
        cs_quote = cs_per_currency.get(quote, pd.Series(0.0, index=ref_ts_series.index))
        cs_diff = cs_base - cs_quote
...
def _print_verdict(agg: dict[str, dict], spread_pip: float) -> None:
    _hdr(f"PHASE 9.16/G-2.5 VERDICT (slippage={spread_pip:.2f}pip; orthogonal features bundle)")
