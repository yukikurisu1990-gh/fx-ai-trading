"""compare_multipair_v26_dynamic_sltp.py - Phase 9.X-M dynamic SL/TP.

Cloned from v22 (risk-sizing). Adds Stage-1 per-pair (tp_mult, sl_mult)
optimization on the FIRST 90-day training window, applied uniformly to
the entire walk-forward (Approach C in design memo).

Stage-1 metric: maximise label "directional edge" |p_tp - p_sl| /
sqrt(p_tp(1-p_tp) + p_sl(1-p_sl)) over the (tp, sl) grid. Picks the
(tp, sl) where the bid/ask-aware TB labels have the strongest
directional bias for that pair.

Stage-2: feed the chosen per-pair (tp, sl) into _build_pair_features
(label generation) and _eval_fold (sizing + PnL replay). Same plumbing
end-to-end so labels and inference use identical (tp, sl).

CLI:
  --enable-dynamic-sltp     toggle (default off; reproduces v22 exactly)
  --sltp-grid "2.0,2.5,...,5.0"   grid sweep values
  --sltp-objective edge|info-ratio (default edge)

Reviewer Rank-5 lever per docs/design/sharpe_improvement_brief.md.
...
def _add_labels_bidask(
    ...
    if n_eff <= 0:
        ...
        df[PNL_LONG_COLUMN] = np.full(n, np.nan, dtype=np.float64)
        df[PNL_SHORT_COLUMN] = np.full(n, np.nan, dtype=np.float64)
        return df
    ...
    # below is unchanged). Vectorised equivalent of the shared
    # traded_direction_pnl.traded_direction_pnl_price helper — identical
    # semantics: SL hit first -> -sl (same-bar TP+SL tie -> SL-first,
    # conservative); else TP hit -> +tp; else horizon-end mark-to-market
    # at the exit side's close (window covers i+1 .. i+horizon). Uses the
    # SAME per-row tp_arr/sl_arr distances the label uses.
    mtm_long = bid_c[horizon : horizon + n_eff] - entry_long_arr
    mtm_short = entry_short_arr - ask_c[horizon : horizon + n_eff]
    long_sl_first = (long_sl_idx >= 0) & ((long_tp_idx < 0) | (long_sl_idx <= long_tp_idx))
    short_sl_first = (short_sl_idx >= 0) & ((short_tp_idx < 0) | (short_sl_idx <= short_tp_idx))
    pnl_long_inner = np.where(long_sl_first, -sl_arr, np.where(long_tp_idx >= 0, tp_arr, mtm_long))
    pnl_short_inner = np.where(
        short_sl_first, -sl_arr, np.where(short_tp_idx >= 0, tp_arr, mtm_short)
    )
    pnl_long = np.full(n, np.nan, dtype=np.float64)
    pnl_short = np.full(n, np.nan, dtype=np.float64)
    pnl_long[:n_eff] = np.where(valid, pnl_long_inner, np.nan)
    pnl_short[:n_eff] = np.where(valid, pnl_short_inner, np.nan)
...
def _optimize_per_pair_sltp(
    ...
    for tp in tp_grid:
        for sl in sl_grid:
            labelled = _add_labels_bidask(df, horizon, tp, sl)
            labels = labelled[LABEL_COLUMN].dropna()
            if labels.empty:
                continue
            n = len(labels)
            n_tp = int((labels == 1).sum())
            n_sl = int((labels == -1).sum())
            n_to = int((labels == 0).sum())
            p_tp = n_tp / n
            p_sl = n_sl / n
            ...
            if score > best_score:
                ...
                best_tp = tp
                best_sl = sl
    return best_tp, best_sl, best_score, all_scores
...
def _eval_fold(
    ...
    min_lot: int = 1000,
    max_size_units: int = 0,
    per_pair_sltp: dict[str, tuple[float, float]] | None = None,
    ...
    for pair in all_pairs:
        ...
        present = pa["present"]
        pip = pair_pip[pair]
        # Phase 9.X-M: per-pair (tp_mult, sl_mult) lookup. Falls back to
        # global tp_mult/sl_mult when per_pair_sltp is None or missing.
        # sl_pip is still needed downstream for ¥-based position sizing.
        _tp_m, sl_m = (
            per_pair_sltp.get(pair, (tp_mult, sl_mult))
            if per_pair_sltp is not None
            else (tp_mult, sl_mult)
        )
        sl_pip = (sl_m * atr) / pip
        ...
        # OWN barrier/horizon outcome. The old mapping scored the label
        # identity (label==0 -> 0.0), which booked masked traded-direction
        # SL hits and timeout exits as 0.0 pips. The pnl columns were
        # computed at label time with the SAME per-pair, per-row TP/SL
        # distances the labels used.
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Phase 9.X-M: Stage-1 per-pair (tp_mult, sl_mult) optimization "
            "on the first training window. Off by default (uses --tp-mult / "
            "--sl-mult globally for all pairs)."
        ),
    )
    parser.add_argument(
        "--sltp-grid",
        type=str,
        default="2.0,2.5,3.0,3.5,4.0,4.5,5.0",
        help=(
            "Phase 9.X-M: comma-separated (tp_mult, sl_mult) grid values. "
            "Symmetric — same grid sweep for both tp and sl. Default 2.0..5.0."
        ),
    )
    parser.add_argument(
        "--sltp-objective",
    ...
    # Phase 9.X-M Stage-1: optimize per-pair (tp_mult, sl_mult) on first
    # training window if dynamic SL/TP is enabled.
    per_pair_sltp: dict[str, tuple[float, float]] = {
        pair: (args.tp_mult, args.sl_mult) for pair in pairs
    }
    if args.enable_dynamic_sltp:
        try:
            tp_grid = [float(v.strip()) for v in args.sltp_grid.split(",") if v.strip()]
            sl_grid = list(tp_grid)  # symmetric grid for now
        except ValueError:
            print(
                f"ERROR: --sltp-grid values must be numeric (got {args.sltp_grid!r})",
                file=sys.stderr,
            )
            return 2
        print(
            f"\nStage-1 dynamic SL/TP optimization "
    ...
    feat_dfs: dict[str, pd.DataFrame] = {}
    for pair in pairs:
        tp_pair, sl_pair = per_pair_sltp[pair]
