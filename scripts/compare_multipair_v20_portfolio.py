"""compare_multipair_v20_portfolio.py - Phase 9.X-G portfolio-optimized SELECTOR.

Cloned from v19 (lookahead-fixed). The ONLY change is in the SELECTOR
rule inside ``_evaluate_fold_phase19``: when ``--rho-max`` < 1.0 is set,
picks are filtered by greedy de-correlation BEFORE forming the K-slot
tally.

Naive top-K (Phase 9.19): sort by confidence, take K.
Phase 9.X-G: sort by confidence, iteratively pick next-best with
|corr(c, already_picked)| < rho_max.

Hypothesis: if pairs are systemically correlated (Phase 9.19 closure),
then naive top-K's K picks all move together — sqrt(K) Sharpe lift
fails. Explicitly de-correlating should restore independence and
crack the Sharpe 0.20 ceiling that has held across 9.17/9.17b/9.19/
9.X-A/9.X-C/9.X-D.

CLI additions vs v19:
  --rho-max FLOAT       Greedy de-correlation threshold. 1.0 (default)
                        disables, reproducing v19 naive top-K.
  --corr-window INT     Rolling correlation window (default 100 m5 bars).
...
def _eval_fold(
    ...
    # ----- SELECTOR over (pair, strategy) — Phase 9.19 Top-K / 9.X-G de-correlation -----
    ...
    # Phase 9.X-G: rolling pairwise correlation tensor for greedy
    # de-correlation. Built only when actually needed (rho_max<1, K>1).
    decorrelate = rho_max < 1.0 and k_use > 1
    corr_tensor: np.ndarray | None = None
    if decorrelate:
        n_pairs = len(all_pairs)
        # Build aligned per-pair log-return matrix from last_close.
        ret_mat = np.zeros((n_lab, n_pairs), dtype=np.float64)
        for i_p, pair in enumerate(all_pairs):
            lc = pair_arrays[pair]["last_close"]
    ...
    if decorrelate:
        # Phase 9.X-G greedy de-correlation: sort by conf, iteratively
        # add next-best with |corr| < rho_max vs already-picked.
        pair_idx_for_cand = [all_pairs.index(p) for p, _s in candidates]
        sorted_idx = np.argsort(-conf_for_pick, axis=0)  # (n_cands, n_lab)
        for b in range(n_lab):
            corr_b = corr_tensor[b]  # (n_pairs, n_pairs)
            picked_pair_idxs: list[int] = []
            slot = 0
            for rank_pos in range(n_cands):
                cidx = int(sorted_idx[rank_pos, b])
                if not bool(traded_mat[cidx, b]):
                    continue
                cand_p_idx = pair_idx_for_cand[cidx]
                # Reject if |corr| with any already-picked pair is too high.
                blocked = False
                for p_idx in picked_pair_idxs:
                    rho = corr_b[cand_p_idx, p_idx]
                    # NaN corr (insufficient history) → fall back to "no info"
                    # i.e. accept the pick (be permissive at fold start).
                    if not np.isnan(rho) and abs(rho) >= rho_max:
                        blocked = True
                        break
                if blocked:
                    continue
                sel_cand_idx_per_rank[slot, b] = cidx
                picked_pair_idxs.append(cand_p_idx)
                slot += 1
                if slot >= k_use:
                    break
    elif not diversify_by_currency:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "Phase 9.X-G greedy de-correlation threshold. When |corr(c, "
            "already_picked)| >= rho_max, candidate c is skipped. "
            "Default 1.0 disables (reproduces v19 naive top-K). "
            "Recommended sweep: 0.3, 0.4, 0.5, 0.6."
        ),
    )
    parser.add_argument(
        "--corr-window",
        ...
        default=100,
        help=(
            "Phase 9.X-G rolling correlation window in m5 bars. Default "
            "100 (~8h). Used only when --rho-max < 1.0."
        ),
    )
