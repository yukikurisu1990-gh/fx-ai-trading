"""stage2_3_horizon_sweep.py — HORIZON sweep at small_boost class_weight (Stage 2.2 best)."""
...
_DATA_DIR = Path("data")
_HORIZONS = [10, 20, 40, 60]

# V2 base + Stage 2.2 best
...
def main() -> None:
    print("="*80)
    print(f"Stage 2.3 HORIZON sweep at small_boost ({_HORIZONS})")
    print(f"  tp={m._TP_MULT}  sl={m._SL_MULT}  conf={m._CONF_THR}  class_weight=small_boost")
    print("="*80, flush=True)
    ...
    for hz in _HORIZONS:
        ...
        results.append((hz, res, elapsed))
        print(f"  HORIZON={hz}  Sharpe={res['sharpe']:+.4f}  hit={res['hit_pct']:5.1f}%  "
    ...
    print("="*80)
    print(f"  {'HORIZON':>8}  {'Sharpe':>8}  {'Hit%':>6}  {'N':>6}  {'PnL(pip)':>10}  {'MaxDD':>10}")
    print("  " + "-"*70)
    for hz, res, _ in sorted(results, key=lambda x: -x[1]['sharpe']):
        print(f"  {hz:8d}  {res['sharpe']:+8.4f}  {res['hit_pct']:6.1f}%  "
