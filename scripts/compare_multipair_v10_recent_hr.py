"""compare_multipair_v10_recent_hr.py — Phase 9.15/F-2 recent hit-rate.

Successor to v9 (`compare_multipair_v9_orthogonal.py`). Same static
orthogonal features (spread/time/volume/regime), and adds the
sequential per-pair `recent_hit_rate_50` feature on top.

Recent-hit-rate requires a 2-pass training loop per fold because
Layer 1's own historical signals are the input:

  Pass 1 (no RH feature):
    1. Train M1 on the fold's train set with whatever bundle is
       active.
    2. Run M1.predict_proba on the full feature DataFrame to get
       per-bar (p_tp, p_sl, signal) for ALL bars (train + test).
    3. Compute recent_hit_rate_50 per pair: for bar i, look at
       Layer 1's last 50 fired signals BEFORE i and take the hit
       fraction (signal direction matches label).

  Pass 2 (with RH feature):
    4. Train M2 on the same train set + the new RH column.
    5. Use M2 for test-set evaluation.

Computational cost: 2x model training per fold (~6 min/cell vs ~3
min/cell in v9). Internal sweep still amortises load + features.

CLI extension over v9
---------------------
    --enable-recent-hit-rate    # Pass 2 enabled (default).
    --no-recent-hit-rate        # Pass 1 only (reproduces v9).

Phase 9.15/F-2 goal: layered on top of F-1's `spread` bundle (the
winner: PnL +13%, MaxDD -17%), does Layer 1's self-feedback lift
PnL further or improve DD? If yes, the feature stays in production;
if not, it's dropped to keep the feature set lean.

Known approximation
-------------------
M1's signals on its own training set are in-sample predictions, so
they are mildly over-optimistic on training bars. The leakage into
recent_hit_rate is bounded but real. A fully OOF approach (K-fold
within the train window) would eliminate it but doubles compute
again. Accept and note in the closure memo.

Requires data/candles_<pair>_M1_365d_BA.jsonl (same as v5/v9).
"""
...
def _build_pair_features(
    ...
    # Phase 9.12/B-2: bid/ask-aware labels.
...
# ---------------------------------------------------------------------------
# F-2: recent hit-rate sequential feature
# ---------------------------------------------------------------------------

_RH_FEATURE_NAME = "recent_hit_rate_50"


def _compute_recent_hit_rate_pair(
    signals: np.ndarray,
    labels: np.ndarray,
    n_window: int = 50,
) -> np.ndarray:
    """Per-bar rolling hit rate over the last ``n_window`` fired signals.

    For each bar i, looks at fired signals at bars [0..i-1] (NOT
    including i), takes the most recent ``n_window`` of them, and
    returns the fraction whose direction matched the realised label
    (long → label==+1, short → label==-1).

    Vectorised via cumulative sums and ``np.searchsorted``. NaN where
    fewer than ``n_window`` prior fires exist.
    """
    n = len(signals)
    if n == 0:
        return np.empty(0, dtype=np.float64)

    # Treat NaN labels as "label not yet known" → not a fired bar.
    label_arr = np.where(np.isnan(labels.astype(np.float64)), 0, labels).astype(np.int8)
    label_present = ~np.isnan(labels.astype(np.float64))

    fired = (signals != 0) & label_present
    correct = ((signals == 1) & (label_arr == 1)) | ((signals == -1) & (label_arr == -1))
    hit = fired & correct

    fired_int = fired.astype(np.int64)
    hit_int = hit.astype(np.int64)
    fired_cum = np.cumsum(fired_int)  # shape (n,)
    hit_cum = np.cumsum(hit_int)

    # fires up through bar i-1 = fired_cum[i-1]; for i=0 use 0.
    fires_before_i = np.concatenate(([0], fired_cum[:-1])).astype(np.int64)
    hit_before_i = np.concatenate(([0], hit_cum[:-1])).astype(np.int64)

    # We want the start index j such that fired_cum[j-1] == fires_before_i - n_window
    # i.e. j = smallest index where fired_cum >= fires_before_i - n_window + 1.
    # np.searchsorted(side='left', target+1) gives that index, equivalently:
    target = fires_before_i - n_window
    j = np.searchsorted(fired_cum, target, side="right")  # smallest k s.t. fired_cum[k] > target
    # hits at the cutoff = hit_cum at j-1 (or 0 if j==0).
    j_safe = np.clip(j - 1, 0, n - 1)
    hit_at_cutoff = np.where(j > 0, hit_cum[j_safe], 0)

    hit_in_window = hit_before_i - hit_at_cutoff
    rate = hit_in_window.astype(np.float64) / n_window

    enough_history = fires_before_i >= n_window
    return np.where(enough_history, rate, np.nan)
...
def _print_verdict(agg: dict[str, dict], spread_pip: float) -> None:
    _hdr(f"PHASE 9.15/F-2 VERDICT (slippage={spread_pip:.2f}pip; orthogonal + RH features)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--features-bundle",
        choices=("baseline", "spread", "time", "volume", "regime", "all"),
        default="all",
        help=(
            "Phase 9.15/F-1 ablation control. 'baseline' reproduces v5 "
            "(spread/time/volume/regime features all dropped). 'spread', "
            "'time', 'volume', 'regime' isolate one group. 'all' (default) "
            "uses all 12 orthogonal features."
        ),
    )
    parser.add_argument(
        "--features-bundles",
        default=None,
        help=(
            "Phase 9.15/F-1 internal sweep. Comma-separated list of cells, "
            "where each cell is one of {baseline, all, <group> [+ <group> ...]}. "
            "Groups: spread, time, volume, regime. "
            "Example: --features-bundles 'spread,spread+volume,spread+volume+regime'. "
            "Loads + builds features ONCE; retrains per cell. "
            "When provided, overrides --features-bundle."
        ),
    )
    parser.add_argument(
        "--enable-recent-hit-rate",
        dest="enable_recent_hit_rate",
        action="store_true",
        default=True,
        help=(
            "Enable Phase 9.15/F-2 sequential per-pair recent_hit_rate_50 "
            "feature (default: enabled). Doubles training time per fold."
        ),
    )
    parser.add_argument(
        "--no-recent-hit-rate",
        dest="enable_recent_hit_rate",
        action="store_false",
        help="Disable F-2 RH feature; reproduces v9 single-pass behaviour.",
    )
    ...
    print("\nBuilding features (per pair, ATR labels + orthogonal features) ...")
    feat_dfs: dict[str, pd.DataFrame] = {}
    for pair in pairs:
    ...
    # Phase 9.15/F-1: bundle resolution. --features-bundles supports
    # multiple comma-separated cells; each cell can be a single group
    # name OR a '+'-joined combo (e.g. 'spread+volume'). Falls back to
    # the legacy single --features-bundle for back-compat.
    if args.features_bundles:
        bundle_specs = [b.strip() for b in args.features_bundles.split(",") if b.strip()]
    else:
        bundle_specs = [args.features_bundle]

    print(f"Ablation cells ({len(bundle_specs)}): {', '.join(bundle_specs)}\n")

    # Always-excluded columns (raw OHLC + bid/ask OHLC + label).
    raw_exclude = {
        "timestamp",
        "open",
        "high",
    ...
    group_to_cols = {
        "spread": _ORTHO_COLS_SPREAD,
        "time": _ORTHO_COLS_TIME,
        "volume": _ORTHO_COLS_VOLUME,
        "regime": _ORTHO_COLS_REGIME,
    }

    cell_summaries: list[tuple[str, dict, dict[str, int]]] = []

    for cell_idx, bundle_spec in enumerate(bundle_specs):
        # Resolve bundle groups for this cell.
        if bundle_spec == "baseline":
            ortho_keep: set[str] = set()
        elif bundle_spec == "all":
            ortho_keep = set(_ORTHO_COLS_ALL)
        else:
            ortho_keep = set()
            for group in bundle_spec.split("+"):
                group = group.strip()
                if group not in group_to_cols:
                    raise ValueError(
                        f"unknown feature group {group!r} in bundle {bundle_spec!r}; "
                        f"valid groups: {sorted(group_to_cols)}"
                    )
                ortho_keep.update(group_to_cols[group])

        ortho_exclude = set(_ORTHO_COLS_ALL) - ortho_keep
        full_exclude = raw_exclude | ortho_exclude
        feature_cols = [c for c in sample.columns if c not in full_exclude]

        _hdr(
            f"CELL {cell_idx + 1}/{len(bundle_specs)}: bundle={bundle_spec!r}  "
        ...
        # F-2: when RH is enabled, the cell uses a 2-pass training scheme.
        # Effective feature_cols for pass-2 inference = base + recent_hit_rate.
        cell_use_rh = args.enable_recent_hit_rate
        feature_cols_p2 = feature_cols + [_RH_FEATURE_NAME] if cell_use_rh else feature_cols
        ...
        for fid, fold in enumerate(folds):
            if fid in retrain_schedule:
                for pair in pairs:
                    ...
                    train_slice = feat_dfs[pair][tr_mask]

                    if cell_use_rh:
                        # Pass 1: train without RH, predict on full df, compute RH.
                        m1 = _train(train_slice, feature_cols, args.n_estimators)
                        x_full = feat_dfs[pair][feature_cols].fillna(0).to_numpy(dtype=np.float64)
                        proba_full = m1.predict_proba(x_full)
                        sig_full = _classify_vec(
                            proba_full[:, 2], proba_full[:, 0], args.conf_threshold
                        )
                        labels_full = feat_dfs[pair][LABEL_COLUMN].to_numpy()
                        rh_arr = _compute_recent_hit_rate_pair(sig_full, labels_full, n_window=50)
                        feat_dfs[pair] = feat_dfs[pair].assign(**{_RH_FEATURE_NAME: rh_arr})

                        # Pass 2: re-train on the same train slice + RH feature.
                        train_slice_p2 = feat_dfs[pair][tr_mask]
                        pair_models[pair] = _train(
                            train_slice_p2, feature_cols_p2, args.n_estimators
                        )
                    else:
                        pair_models[pair] = _train(train_slice, feature_cols, args.n_estimators)
        ...
        cell_summaries.append((bundle_spec, agg, pair_select_totals))

    # Cross-cell summary (only when multi-cell sweep).
    if len(cell_summaries) > 1:
        _hdr("MULTI-CELL ABLATION SUMMARY (SELECTOR)")
        print(
            f"  {'Cell':<28} {'NetSharpe':>10} {'NetPnL(pip)':>13} "
            f"{'MaxDD(pip)':>11} {'DD%PnL':>8} {'Trades':>10}"
        )
        print("  " + "-" * 90)
        for spec, agg, _ in cell_summaries:
            sel = agg["SELECTOR"]
            print(
                f"  {spec:<28} {sel['overall_sharpe_net']:>10.3f} "
                f"{sel['net_pnl']:>13.1f} {sel['max_drawdown_pip']:>11.1f} "
                f"{sel['max_dd_pct_of_pnl']:>7.1f}% {sel['total_trades']:>10,}"
            )
