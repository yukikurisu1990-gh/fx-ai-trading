"""compare_multipair_v6_meta.py — Phase 9.12/B-3 meta-labeling on top of v5.

Successor to v5 (`compare_multipair_v5_bidask.py`). Adds a Lopez de
Prado-style meta-labeling layer ("Layer 2") on top of v5's primary
classifier ("Layer 1"). Layer 2's job is to *whiten* Layer 1's signals
— for each non-no-trade signal Layer 1 emits, Layer 2 predicts the
probability that the trade will actually be profitable (TP fires
before SL). Trades whose Layer 2 probability is below a threshold are
suppressed.

Why this should help vs v5
--------------------------
v5 cleared the SOFT GO threshold (SELECTOR net Sharpe 0.160) but not
the GO threshold (>= 0.20). Two things suggest meta-labeling can
close that gap:

  1. v5's signal rate is already low (3.5% on SELECTOR), so we are
     not relying on volume to drag the average. Marginal Sharpe
     comes from raising the *quality* of each retained signal.
  2. The primary model's predict_proba is cluster-skewed: many
     signals fire from very high-confidence bars whose actual hit
     rate is mediocre. Meta-labeling typically picks up second-order
     features (regime, recent volatility, recent hit rate) that
     correlate with the primary model's miss pattern.

Walk-forward protocol
---------------------
For each fold:

  1. Train Layer 1 on `[train_start, train_end)` with v5's bid/ask-
     aware ATR triple-barrier label.
  2. Run Layer 1 inference on the *training* slice to produce, per
     bar, `(p_tp, p_sl, primary_signal)`.
  3. Filter to bars where `primary_signal != no_trade`. For each, the
     binary meta-label is:
        meta_label = 1 iff (signal == long  AND label ==  1)
                          OR (signal == short AND label == -1)
                     = 0 otherwise (timeout or wrong direction)
  4. Train Layer 2 on the filtered set. Inputs = original features
     + Layer 1's `p_tp` and `p_sl` as two extra columns. Output =
     binary meta-label.
  5. At test time: get Layer 1 signal; if non-no-trade, run Layer 2;
     keep the trade iff `meta_proba >= --meta-threshold` (default
     0.50).

Caveat
------
Walk-forward bias: training Layer 2 on Layer 1's *training* set
inference is fine — both Layer 1 and Layer 2 see only `[train_start,
train_end)` data. The Layer 2 binary label uses the (already-
computed) primary triple-barrier label, which is forward-looking
within the same training window — not a leakage concern because
both layers are fit on the same window.

Default-config Phase 9.12 gate target: SELECTOR net Sharpe >= 0.20
at slippage=0.0pip. Achieving this would lift the Phase 9.10 GO
flag and unblock Phase 9.11.

Usage
-----
    python scripts/compare_multipair_v6_meta.py \
        --tp-mult 1.5 --sl-mult 1.0 --horizon 20 \
        --slippage-pip 0.0 --conf-threshold 0.50 \
        --meta-threshold 0.50

    # Disable meta layer (reproduces v5):
    python scripts/compare_multipair_v6_meta.py --no-meta
"""
...
def _train_meta(
    ...
    """Fit Layer 2 (meta-labeler) on Layer 1's training-set predictions.

    Layer 2 inputs   = original features + Layer 1's (p_tp, p_sl).
    Layer 2 label    = 1 iff Layer 1's signal would have been profitable
                       (long+TP or short+TP); 0 otherwise.
    Layer 2 training = only bars where Layer 1 emitted a non-no-trade
                       signal at the given conf threshold.

    Returns ``None`` if no Layer 1 trade fired in the training window
    (caller must fall back to Layer 1 only for that pair).
    """
    labeled = train_df.dropna(subset=[LABEL_COLUMN])
    if labeled.empty:
        return None
    x_primary = labeled[feature_cols].fillna(0).to_numpy(dtype=np.float64)
    proba = primary_model.predict_proba(x_primary)
    p_tp = proba[:, 2]
    p_sl = proba[:, 0]
    primary_sig = _classify_vec(p_tp, p_sl, ml_threshold)
    primary_label = labeled[LABEL_COLUMN].to_numpy(dtype=np.int8)

    trade_mask = primary_sig != 0
    if not trade_mask.any():
        return None

    # Binary meta-label: did Layer 1 pick the right direction?
    correct = np.zeros_like(primary_sig, dtype=np.int8)
    correct[(primary_sig == 1) & (primary_label == 1)] = 1
    correct[(primary_sig == -1) & (primary_label == -1)] = 1
    meta_y = correct[trade_mask].astype(np.int64).tolist()

    # Layer 2 features = original features + p_tp + p_sl on trade bars.
    meta_x = np.column_stack([x_primary[trade_mask], p_tp[trade_mask], p_sl[trade_mask]]).tolist()

    if len(set(meta_y)) < 2:
        # Degenerate: all trades were correct or all wrong — Layer 2 has
        # nothing to learn. Skip it.
        return None
...
def _meta_keep_proba(
    meta_model: lgb.LGBMClassifier | None,
    feat_arr: np.ndarray,
    p_tp: np.ndarray,
    p_sl: np.ndarray,
) -> np.ndarray:
    """Return Layer 2's `keep` probability per bar.

    Falls back to all-1.0 (always keep) when Layer 2 was not trained,
    matching v5's "no meta layer" behaviour.
    """
    n = len(p_tp)
    if meta_model is None:
        return np.ones(n, dtype=np.float64)
    meta_x = np.column_stack([feat_arr, p_tp, p_sl])
    proba = meta_model.predict_proba(meta_x)
    # binary classifier: column 1 = P(meta_label == 1) = P(profitable)
...
def _eval_fold(
    ...
    sl_mult: float,
    rng: random.Random,
    pair_meta_models: dict[str, lgb.LGBMClassifier | None] | None = None,
    meta_threshold: float = 0.50,
) -> tuple[dict, dict[str, int]]:
    """Optimised per-fold evaluation (Phase 9.12 perf pass + B-3 meta-labeling).

    When ``pair_meta_models`` is supplied (one Layer-2 model per pair —
    can be ``None`` per pair if Layer 2 wasn't trained), each pair's
    primary signal is suppressed unless Layer 2's ``keep`` probability
    is at least ``meta_threshold``. Set ``pair_meta_models=None`` to
    reproduce v5 behaviour exactly.
    """
    ...
    for pair in all_pairs:
        ...
        # Stash feat_arr too — Layer 2 needs it at inference time.
        pair_arrays[pair] = {
            "p_tp": p_tp,
            "p_sl": p_sl,
            "label": label_arr,
    ...
    pair_conf: dict[str, np.ndarray] = {}  # max(p_tp, p_sl) (raw L1 confidence)
    ...
    for pair in all_pairs:
        ...
        # B-3 meta filter: suppress signals whose Layer 2 keep-proba is
        # below the threshold. When pair_meta_models is None or its entry
        # is None for this pair, all signals pass (matches v5 behaviour).
        if pair_meta_models is not None and sig.any():
            meta_model = pair_meta_models.get(pair)
            if meta_model is not None:
                keep_proba = _meta_keep_proba(meta_model, pa["feat_arr"], pa["p_tp"], pa["p_sl"])
                drop_mask = (sig != 0) & (keep_proba < meta_threshold)
                sig = np.where(drop_mask, 0, sig).astype(np.int8)
...
def _print_verdict(agg: dict[str, dict], spread_pip: float) -> None:
    _hdr(
        f"PHASE 9.12/B-3 VERDICT (slippage={spread_pip:.2f}pip; "
        "bid/ask in labels; meta-labeling layer)"
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="compare_multipair_v6_meta",
        description="Phase 9.12/B-3 meta-labeling on top of v5 bid/ask labels.",
    )
    ...
    parser.add_argument(
        ...
        default=0.50,
        help=(
            "Layer 2 keep-probability threshold. Trades whose Layer 2 "
            "P(profitable) is below this are suppressed. Default 0.50."
        ),
    )
    parser.add_argument(
        "--no-meta",
        action="store_true",
        help="Disable the meta-labeling layer (reproduces v5 behaviour exactly).",
    )
    ...
    if args.no_meta:
        print("  meta-labeling DISABLED (--no-meta) — reproducing v5 behaviour")
    else:
        print(
            f"  meta-labeling ENABLED  threshold={args.meta_threshold:.2f}  "
            "(B-3: Layer 2 whitens Layer 1 signals)"
        )
    rng = random.Random(args.seed)
    pair_models: dict[str, lgb.LGBMClassifier] = {}
    pair_meta_models: dict[str, lgb.LGBMClassifier | None] = {}
    ...
    for fid, fold in enumerate(folds):
        if fid in retrain_schedule:
            for pair in pairs:
                ...
                if args.no_meta:
                    pair_meta_models[pair] = None
                else:
                    # Layer 2: trained on Layer 1's training-set predictions.
                    pair_meta_models[pair] = _train_meta(
                        primary_model=pair_models[pair],
                        train_df=train_slice,
                        feature_cols=feature_cols,
                        ml_threshold=args.conf_threshold,
        ...
        results, sel_counts = _eval_fold(
            ...
            args.sl_mult,
            rng,
            pair_meta_models=None if args.no_meta else pair_meta_models,
