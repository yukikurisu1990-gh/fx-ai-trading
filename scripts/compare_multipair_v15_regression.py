"""compare_multipair_v15_regression.py - Phase 9.X-A return-based regression labels.

Successor to v14 (Top-K SELECTOR). Identical load + features + cross-pair
pipeline; the difference is **the model class and training target**:

    v14: LGBMClassifier on triple-barrier label (∈ {-1, 0, 1})
    v15: LGBMRegressor  on label_return (∈ R, close-to-close at horizon)

Signal generation at eval time:
    long  if y_hat ≥ +abs_thresh
    short if y_hat ≤ -abs_thresh
    else  no_trade
Where abs_thresh = 50th percentile of |y_hat| on training fold.
Confidence = |y_hat| / (2 × abs_thresh), capped at 1.0.

PnL is still computed via triple-barrier mechanism (apples-to-apples
with v9/v14 baselines). Only signal generation differs.

Top-K SELECTOR (from v14) is preserved; --top-ks works the same way
with regression-based confidence.
...
LABEL_RETURN_COLUMN = "label_return"  # Phase 9.X-A: regression target
...
def _add_label_return(df: pd.DataFrame, horizon: int) -> pd.DataFrame:
    """Phase 9.X-A: regression label = close-to-close return at horizon.

    Computed on mid close (close column) consistent with feature OHLC.
    NaN at the tail where i + horizon would exceed bounds.
    """
    closes = df["close"].to_numpy(dtype=np.float64)
    n = len(closes)
    ...
    for i in range(n - horizon):
        if closes[i] > 0 and np.isfinite(closes[i + horizon]):
            rets[i] = (closes[i + horizon] - closes[i]) / closes[i]
    df = df.copy()
    df[LABEL_RETURN_COLUMN] = rets
    return df
...
def _build_pair_features(
    ...
    # Phase 9.15/F-1: orthogonal features (added BEFORE labels).
    df = _add_orthogonal_features(df, instrument)
    # Phase 9.17 TB label (kept for eval-time PnL — apples-to-apples).
    df = _add_labels_bidask(df, horizon, tp_mult, sl_mult)
    # Phase 9.X-A regression label.
    df = _add_label_return(df, horizon)
    df = df.iloc[50:].copy()
    df = df.reset_index()
    return df
...
def _train(
    ...
    """Phase 9.X-A: train a regressor on label_return.

    Returns (model, abs_pred_threshold). The threshold is the
    ``threshold_pct``-th percentile of |y_hat| on the training set —
    bars whose absolute predicted return falls below this are no-trade.
    """
    labeled = train_df.dropna(subset=[LABEL_RETURN_COLUMN])
    x = labeled[feature_cols].fillna(0).to_numpy(dtype=np.float64)
    y = labeled[LABEL_RETURN_COLUMN].to_numpy(dtype=np.float64)
    params = {**DEFAULT_PARAMS, "n_estimators": n_estimators, "verbose": -1}
    model = lgb.LGBMRegressor(**params)
    model.fit(x, y)
    train_pred = model.predict(x)
    threshold = float(np.quantile(np.abs(train_pred), threshold_pct / 100.0))
...
def _eval_fold(
    pair_models: dict[str, tuple[lgb.LGBMRegressor, float]],
    ...
    """Phase 9.X-A regression-based eval (preserves Phase 9.19 Top-K semantics).

    pair_models is now a dict[pair, (LGBMRegressor, abs_pred_threshold)].
    The per-bar signal is sign(predicted_return) when |predicted_return|
    exceeds the per-pair training threshold. Confidence is the
    magnitude-normalised-to-[0,1] absolute predicted return.
    ...
    for pair in all_pairs:
        ...
        # Phase 9.X-A: regressor predicts return; threshold from training fold.
        if _STRATEGY_LGBM in cell_strategies:
            feat_arr = aligned[feature_cols].fillna(0.0).to_numpy(dtype=np.float64)
            model_obj, abs_thresh = pair_models[pair]
            if present.any():
                y_hat = model_obj.predict(feat_arr).astype(np.float64)
            else:
                y_hat = np.zeros(n_lab, dtype=np.float64)
        else:
            y_hat = np.zeros(n_lab, dtype=np.float64)
            abs_thresh = 1.0
    ...
    for pair in all_pairs:
        ...
        if _STRATEGY_LGBM in cell_strategies:
            # Phase 9.X-A: regression-based signal.
            # Long when y_hat >= +abs_thresh; short when y_hat <= -abs_thresh.
            # Confidence = |y_hat| / (2 * abs_thresh), capped at 1.0.
            y_hat = pa["y_hat"]
            abs_thresh = pa["abs_pred_threshold"]
            long_mask = y_hat >= abs_thresh
            short_mask = y_hat <= -abs_thresh
            sig_l = np.zeros(n_lab, dtype=np.int8)
            sig_l[long_mask] = 1
            sig_l[short_mask] = -1
            sig_l = np.where(present, sig_l, 0).astype(np.int8)
            denom = max(2.0 * abs_thresh, 1e-12)
            conf_l = np.minimum(np.abs(y_hat) / denom, 1.0)
            conf_l = np.where(present, conf_l, 0.0)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=50.0,
        help=(
            "Phase 9.X-A: percentile of training |y_hat| used as the absolute "
            "predicted-return threshold. Bars with |y_hat| < this threshold "
            "produce no_trade. Default 50 (mid-percentile). Higher percentiles "
            "(70, 90, 95) cut trade rate aggressively to match LGBM classifier "
            "behavior at conf>=0.5."
        ),
    )
    parser.add_argument(
    ...
    print(
        f"Top-K sweep: K in {top_ks} | diversify_by_currency={diversify_str} | "
        f"regression_threshold_pct={args.regression_threshold_pct}\n"
    )
    ...
    raw_exclude = {
        ...
        "ask_c",
        LABEL_COLUMN,
        # Phase 9.X-A: regression target derived from FUTURE close;
        # must not enter the model as a feature.
        LABEL_RETURN_COLUMN,
    }
