"""compare_multipair_v17_lstm.py - Phase 9.X-C/M-1 LSTM Mode A (full replacement).

Successor to v16 (Phase 9.X-B feature groups). Replaces the LightGBM
3-class classifier with a 2-layer LSTM trained on the same 21-feature
sequence (15 baseline + 6 mtf when --feature-groups mtf). Per
Phase 9.X-C/M-1 design memo.

Mode A — full replacement: LSTM output directly substitutes LightGBM's
predict_proba in the lgbm strategy slot. SELECTOR rule (Top-K,
diversification) and the rule-based MR/BO strategies are unchanged.

Architecture defaults:
  Model:           2-layer LSTM, 64 hidden, dropout 0.2 (~35k params)
  Sequence length: 100 m5 bars (configurable via --lstm-sequence)
  Target:          3-class TB label (matches LightGBM target encoding)
  Training:        PyTorch + cross-entropy loss + Adam optimizer
  Device:          CUDA preferred; falls back to CPU
  Per-fold cache:  same retrain cadence as LightGBM (3 retrains/39 folds)

CLI additions vs v16:
  --use-lstm                  Enable LSTM Mode A (default off → LightGBM)
  --lstm-sequence INT         Sequence length (default 100)
  --lstm-epochs INT           Training epochs per fold (default 10)
  --lstm-hidden INT           Hidden units (default 64)
  --lstm-batch-size INT       Mini-batch size (default 256)
  --lstm-lr FLOAT             Learning rate (default 1e-3)

Default behaviour (without --use-lstm) is exact v16 reproduction.

Verdict gates (raised vs Phase 9.X-B mtf benchmark):
  GO          - Sharpe ≥ 0.18 AND PnL ≥ 1.10 × baseline AND DD%PnL ≤ 5%
  PARTIAL GO  - Sharpe ≥ 0.174 (Phase 9.X-B mtf K=3 benchmark)
  STRETCH GO  - Sharpe ≥ 0.20 (unblocks Phase 9.11)
  NO ADOPT    - Sharpe < 0.174

Phase 9.X-B mtf benchmark = (Sharpe 0.174, PnL 15,118 pip, DD%PnL 1.8%
at K=3 lgbm_only with --feature-groups mtf).
...
# Phase 9.X-C/M-1 LSTM defaults.
_LSTM_DEFAULT_SEQUENCE = 100
_LSTM_DEFAULT_HIDDEN = 64
_LSTM_DEFAULT_LAYERS = 2
_LSTM_DEFAULT_DROPOUT = 0.2
_LSTM_DEFAULT_EPOCHS = 10
_LSTM_DEFAULT_BATCH_SIZE = 256
_LSTM_DEFAULT_LR = 1e-3
_LSTM_DEVICE = "cuda" if torch.cuda.is_available() else "cpu"


class LSTMClassifier(nn.Module):
    """Phase 9.X-C/M-1 — 2-layer LSTM 3-class classifier on 21-feature sequence.

    Output: logits over {SL, timeout, TP} (matches _LABEL_ENCODE order).
    """

    def __init__(
        self,
        n_features: int,
        hidden_dim: int = _LSTM_DEFAULT_HIDDEN,
        n_layers: int = _LSTM_DEFAULT_LAYERS,
        dropout: float = _LSTM_DEFAULT_DROPOUT,
        n_classes: int = 3,
    ) -> None:
        super().__init__()
        self.lstm = nn.LSTM(
...
def _build_sequences_train(
    ...
    For each row i where label_tb is non-null AND i >= sequence_length-1,
    take features[i-sequence_length+1 : i+1] as input and the encoded
    label as target.
    """
    x_full = df[feature_cols].fillna(0.0).to_numpy(dtype=np.float32)
    labels_raw = df[LABEL_COLUMN].to_numpy()
    valid_mask = ~pd.isna(labels_raw)
    candidate_idx = np.where(valid_mask)[0]
    candidate_idx = candidate_idx[candidate_idx >= sequence_length - 1]
    if candidate_idx.size == 0:
        return (
            np.zeros((0, sequence_length, x_full.shape[1]), dtype=np.float32),
            np.zeros(0, dtype=np.int64),
        )
    sequences = np.empty((candidate_idx.size, sequence_length, x_full.shape[1]), dtype=np.float32)
    targets = np.empty(candidate_idx.size, dtype=np.int64)
    for out_i, src_i in enumerate(candidate_idx):
        sequences[out_i] = x_full[src_i - sequence_length + 1 : src_i + 1]
        targets[out_i] = _LABEL_ENCODE[int(labels_raw[src_i])]
    return sequences, targets


def _build_sequences_inference(
    test_indices: np.ndarray,
    full_features: np.ndarray,
    sequence_length: int,
) -> np.ndarray:
    """Build sequences for inference at given absolute indices in `full_features`.

    For each idx in `test_indices`, returns features[idx-sequence_length+1:idx+1].
    Indices < sequence_length-1 are zero-padded at the front.
    """
    n_features = full_features.shape[1]
    sequences = np.zeros((test_indices.size, sequence_length, n_features), dtype=np.float32)
    for out_i, src_i in enumerate(test_indices):
        start = src_i - sequence_length + 1
        if start < 0:
            sequences[out_i, -src_i - 1 :] = full_features[: src_i + 1]
        else:
            sequences[out_i] = full_features[start : src_i + 1]
    return sequences
...
def _train_lstm(
    ...
    """Phase 9.X-C/M-1 — train LSTMClassifier on the 90-day train window.

    Returns the trained model in eval mode, on the requested device.
    Empty / insufficient training data → returns an untrained model
    (predictions will be near-uniform across classes).
    """
    x_arr, y_arr = _build_sequences_train(train_df, feature_cols, sequence_length)
    ...
    if x_arr.shape[0] < batch_size:
        # Insufficient data; return untrained model.
        model.eval()
        return model
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    # Phase 9.X-C/M-1: inverse-frequency class weights to counter the
    # ~84% timeout label imbalance. Without weighting the model learns
    # to predict timeout for everything → no trades fire.
    class_counts = np.bincount(y_arr, minlength=3).astype(np.float64)
    class_weights = np.where(
        class_counts > 0,
        x_arr.shape[0] / (3.0 * class_counts),
        1.0,
    )
    ...
    # Pin memory only when CUDA is available (CPU pinning is a noop / warning).
    pin = device == "cuda"
    loader = DataLoader(
        dataset,
...
def _predict_lstm_proba(
    model: LSTMClassifier,
    full_features: np.ndarray,
    test_absolute_indices: np.ndarray,
    sequence_length: int,
    batch_size: int,
    device: str,
) -> np.ndarray:
    """Phase 9.X-C/M-1 — return (n_test, 3) softmax probabilities.

    `full_features` is the 2-D feature matrix for the entire pair history
    on the base timestamp grid (n_bars, n_features). `test_absolute_indices`
    are the row indices we need predictions for.
    """
    if test_absolute_indices.size == 0:
        return np.zeros((0, 3), dtype=np.float64)
    sequences = _build_sequences_inference(test_absolute_indices, full_features, sequence_length)
    x_tensor = torch.from_numpy(sequences)
    pin = device == "cuda"
    model.eval()
    out = np.empty((test_absolute_indices.size, 3), dtype=np.float64)
    with torch.no_grad():
        for start in range(0, x_tensor.shape[0], batch_size):
            stop = min(start + batch_size, x_tensor.shape[0])
            batch = x_tensor[start:stop].to(device, non_blocking=pin)
            logits = model(batch)
            proba = torch.softmax(logits, dim=-1).cpu().numpy()
            out[start:stop] = proba
    return out
...
def _eval_fold(
    ...
    pair_lstm_proba: dict[str, pd.DataFrame] | None = None,
    ...
    # Phase 9.X-C/M-1: in LSTM mode pair_models is empty; derive from
    # pair_test_dfs which is always populated for both LGBM and LSTM.
    all_pairs = list(pair_test_dfs.keys()) if pair_test_dfs else list(pair_models.keys())
    ...
    for pair in all_pairs:
        ...
        # Phase 9.X-C/M-1 — when pair_lstm_proba is supplied, the LSTM
        # output replaces LightGBM's predict_proba (Mode A full replacement).
        if _STRATEGY_LGBM in cell_strategies:
            if pair_lstm_proba is not None:
                proba_df = pair_lstm_proba.get(pair)
                if proba_df is not None and not proba_df.empty:
                    aligned_proba = proba_df.reindex(base_ts)
                    p_sl = aligned_proba["P_sl"].fillna(0.0).to_numpy(dtype=np.float64)
                    p_tp = aligned_proba["P_tp"].fillna(0.0).to_numpy(dtype=np.float64)
                else:
                ...
                if present.any():
                    proba = pair_models[pair].predict_proba(feat_arr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Phase 9.X-C/M-1 — replace LightGBM with 2-layer LSTM as the "
            "lgbm strategy slot (Mode A full replacement). MR/BO strategies "
            "and SELECTOR rule unchanged. Default off → v16 reproduction."
        ),
    )
    parser.add_argument(
        "--lstm-sequence",
        type=int,
        default=_LSTM_DEFAULT_SEQUENCE,
        help="LSTM input sequence length in m5 bars (default 100).",
    )
    parser.add_argument(
        "--lstm-hidden",
        type=int,
        default=_LSTM_DEFAULT_HIDDEN,
        help="LSTM hidden units (default 64).",
    )
    parser.add_argument(
        "--lstm-epochs",
        type=int,
        default=_LSTM_DEFAULT_EPOCHS,
        help="LSTM training epochs per fold (default 10).",
    )
    parser.add_argument(
        "--lstm-batch-size",
        type=int,
        default=_LSTM_DEFAULT_BATCH_SIZE,
        help="LSTM mini-batch size (default 256).",
    )
    parser.add_argument(
        "--lstm-lr",
        type=float,
        default=_LSTM_DEFAULT_LR,
        help="LSTM Adam learning rate (default 1e-3).",
    )
    parser.add_argument(
        "--feature-groups",
        default="",
    ...
    # Phase 9.X-C/M-1 LSTM model store (used only when --use-lstm).
    pair_lstm_models: dict[str, LSTMClassifier] = {}
    # Pre-computed LSTM proba per pair per fold; reset each fold.
    pair_lstm_proba: dict[str, pd.DataFrame] = {}
    if args.use_lstm:
        print(
            f"Phase 9.X-C/M-1 LSTM Mode A enabled: device={_LSTM_DEVICE}, "
            f"sequence={args.lstm_sequence}, hidden={args.lstm_hidden}, "
            f"epochs={args.lstm_epochs}, batch={args.lstm_batch_size}, "
            f"lr={args.lstm_lr}"
        )
    ...
    for fid, fold in enumerate(folds):
        if fid in retrain_schedule:
            for pair in pairs:
                ...
                if args.use_lstm:
                    pair_lstm_models[pair] = _train_lstm(
                        train_df=feat_dfs[pair][tr_mask],
                        feature_cols=feature_cols,
                        sequence_length=args.lstm_sequence,
                        hidden_dim=args.lstm_hidden,
                        n_layers=_LSTM_DEFAULT_LAYERS,
                        dropout=_LSTM_DEFAULT_DROPOUT,
                    ...
                    pair_models[pair] = _train(
                        feat_dfs[pair][tr_mask], feature_cols, args.n_estimators
                    )
        pair_test_dfs: dict[str, pd.DataFrame] = {}
        for pair in pairs:
            ts = feat_dfs[pair]["timestamp"]
        ...
        if args.use_lstm:
            # Pre-compute LSTM proba per pair for the test window. Build
            # sequences from full feat_dfs[pair] so sequence-prefix bars
            # outside the test window are available.
            pair_lstm_proba = {}
            for pair in pairs:
                model_obj = pair_lstm_models.get(pair)
                if model_obj is None:
                    continue
                pair_full_df = feat_dfs[pair]
                pair_full_ts = pair_full_df["timestamp"].to_numpy()
                test_mask_full = (pair_full_ts >= fold["test_start"]) & (
                    pair_full_ts < fold["test_end"]
                )
                test_indices_in_pair = np.where(test_mask_full)[0]
                if test_indices_in_pair.size == 0:
                    pair_lstm_proba[pair] = pd.DataFrame(
                        columns=["P_sl", "P_to", "P_tp"], dtype=np.float64
                    )
                    continue
                full_feat_arr = pair_full_df[feature_cols].fillna(0.0).to_numpy(dtype=np.float32)
                proba = _predict_lstm_proba(
                    model=model_obj,
                    full_features=full_feat_arr,
                    test_absolute_indices=test_indices_in_pair,
                    sequence_length=args.lstm_sequence,
                    batch_size=max(args.lstm_batch_size, 512),
                    device=_LSTM_DEVICE,
                )
                test_ts = pair_full_ts[test_indices_in_pair]
                pair_lstm_proba[pair] = pd.DataFrame(
                    proba,
                    index=test_ts,
                    columns=["P_sl", "P_to", "P_tp"],
                )
        ...
        for cell_name, k in sweep_keys:
            ...
            results, pair_counts, strat_counts, pnl_series, per_pick = _eval_fold(
                ...
                pair_lstm_proba=pair_lstm_proba if args.use_lstm else None,
            )
            key = (cell_name, k)
