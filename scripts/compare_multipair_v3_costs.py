"""compare_multipair_v3_costs.py — Phase 9.10 cost-aware multipair backtest.

Successor to the earlier zero-cost ``compare_multipair_v2`` probe.  Adds
two things v2 could not model:

  1. **Spread cost** — each closed trade pays the full bid/ask spread
     (long pays ask at entry, receives bid at exit; symmetric for short).
     Spread is specified in pips via ``--spread-pip`` and is common to
     all pairs (grid search in A-6 will sweep this).
  2. **Entry/exit side realism** — labels remain mid-based (triple
     barrier on mid close) because that keeps the signal space
     comparable with v2; PnL is computed as ``gross_mid - spread`` per
     executed trade.  Timeout trades also incur the spread (entered +
     exited), matching what a live trader would actually pay.

Why we still label on mid instead of bid/ask-aware barriers
-----------------------------------------------------------
For the Go/No-Go judgment we only need to know whether the v2 signal
structure survives spread.  Re-barring on bid-high/bid-low for long,
ask-low/ask-high for short would make the v3 score strictly closer to
zero (barriers tightened by half-spread on each side).  Deferred to
Phase 9.12 where dynamic ATR barriers are introduced together.

Outputs a 4-strategy comparison table (EURUSD_ML, SELECTOR, EQUAL_AVG,
RANDOM) with both gross (v2-equivalent) and net (spread-adjusted) Sharpe
so the cost gap is explicit per strategy.

Usage
-----
    python scripts/compare_multipair_v3_costs.py \
        --spread-pip 1.0 \
        --conf-threshold 0.50 \
        --tp-pip 3 --sl-pip 2 --horizon 20

Default behaviour matches v2 (TP=3 / SL=2 / horizon=20, conf=0.50) so the
v3 Sharpe at ``--spread-pip 0`` reproduces v2 exactly — giving a clean
spread-zero sanity check before exploring cost regimes.
"""
...
# ---------------------------------------------------------------------------
# Data loading (mid close + optional bid/ask OHLC pass-through)
# ---------------------------------------------------------------------------
...
def _load_mid(path: Path) -> pd.DataFrame:
    """Load M-mode (o/h/l/c) JSONL into a mid-OHLC DataFrame indexed by ts."""
    rows = []
...
def _load_ba(path: Path) -> pd.DataFrame:
    """Load BA-mode JSONL into a DataFrame with bid_c/ask_c columns + mid close.

    The mid close is synthesized as (bid_c + ask_c) / 2 so downstream
    feature builders treat BA files like M files for feature purposes.
    """
    rows = []
    with path.open(encoding="utf-8") as fh:
        for line in fh:
...
def _pick_file(pair: str) -> tuple[Path, str]:
    """Prefer the BA file for observed-spread diagnostics; fall back to M."""
...
# ---------------------------------------------------------------------------
# Feature engineering (identical to v2 for apples-to-apples comparison)
# ---------------------------------------------------------------------------
...
def _add_labels(df: pd.DataFrame, horizon: int, tp: float, sl: float) -> pd.DataFrame:
...
def _build_pair_features(df_m1: pd.DataFrame, horizon: int, tp: float, sl: float) -> pd.DataFrame:
    df = _add_m1_features(df_m1)
    df = _add_upper_tf(df, "5min", "m5")
    df = _add_upper_tf(df, "15min", "m15")
    df = _add_upper_tf(df, "1h", "h1")
    df = _add_labels(df, horizon, tp, sl)
    df = df.iloc[50:].copy()
    df = df.reset_index()
    return df
...
def _get_ev(
    model: lgb.LGBMClassifier, row: pd.Series, feature_cols: list[str], threshold: float
) -> tuple[str, float]:
...
# ---------------------------------------------------------------------------
# Cost-aware PnL + fold evaluation
# ---------------------------------------------------------------------------


def _gross_pnl_pips(sig: str, label: int, tp_pip: float, sl_pip: float) -> float | None:
    """Mid-based gross PnL for a trade, expressed in pips."""
    if sig == "no_trade":
        return None
    if sig == "long":
...
def _net_pnl_pips(gross: float | None, spread_pip: float) -> float | None:
    """Deduct the full round-trip spread from every executed trade.

    Timeout trades (gross=0) are included — the trader still paid ask at
    entry and received bid at exit even if the mid didn't move.
    """
    if gross is None:
        return None
...
def _eval_fold(
    ...
    # Track both gross and net pip series for each strategy.
    acc: dict[str, dict] = {
    ...
    for _, base_row in base_df.iterrows():
        ...
        base_gross = _gross_pnl_pips(base_sig, int(base_row[LABEL_COLUMN]), tp_pip, sl_pip)
        _record("EURUSD_ML", base_gross)

        # All pairs at this timestamp
        pair_signals: dict[str, tuple[str, float, int]] = {}
        for pair, model in pair_models.items():
            ...
            pair_signals[pair] = (sig, conf, label)
        ...
        # SELECTOR: max-EV active pair
        active = [(p, d) for p, d in pair_signals.items() if d[0] != "no_trade"]
        if active:
            best = max(active, key=lambda x: x[1][1])[0]
            sig, _, label = pair_signals[best]
            _record("SELECTOR", _gross_pnl_pips(sig, label, tp_pip, sl_pip))
            pair_select_counts[best] += 1
        else:
            acc["SELECTOR"]["n"] += 1

        # EQUAL_AVG: mean pnl across all signalling pairs
        traded_gross: list[float] = []
        for sig, _, label in pair_signals.values():
            g = _gross_pnl_pips(sig, label, tp_pip, sl_pip)
            if g is not None:
                traded_gross.append(g)
        if traded_gross:
            mean_gross = sum(traded_gross) / len(traded_gross)
            mean_net = mean_gross - spread_pip  # every averaged leg pays spread
            ea = acc["EQUAL_AVG"]
            ea["gross"].append(mean_gross)
            ea["net"].append(mean_net)
            if mean_net > 0:
                ea["w"] += 1
            elif mean_net < 0:
                ea["l"] += 1
        ...
        sig, _, label = pair_signals[rp]
        _record("RANDOM", _gross_pnl_pips(sig, label, tp_pip, sl_pip))
...
def _aggregate(fold_results: list[dict]) -> dict:
    ...
    for s in strats:
        ...
        agg[s] = {
            ...
            "mean_fold_net_sharpe": float(mean_sh),
            "median_fold_net_sharpe": med_sh,
            "std_fold_net_sharpe": std_sh,
            "overall_gross_sharpe": float(mean_gross),
...
def _print_comparison(agg: dict[str, dict], spread_pip: float) -> None:
    _hdr(f"COMPARISON SUMMARY (spread={spread_pip:.2f} pip round-trip)")
    print(
...
def _print_verdict(agg: dict[str, dict], spread_pip: float) -> None:
    _hdr(f"PHASE 9.10 COST-AWARE VERDICT (spread={spread_pip:.2f} pip)")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="compare_multipair_v3_costs",
        description="Phase 9.10 cost-aware multipair backtest.",
    )
    parser.add_argument(
        "--pairs", default=",".join(DEFAULT_PAIRS), help="Comma-separated list of OANDA pair codes."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Round-trip bid/ask spread in pips (common to all pairs).",
    )
    ...
    print(
        f"Base: {base_pair} | TP={args.tp_pip}pip / SL={args.sl_pip}pip / "
    ...
    # Build features (per pair) + cross-pair features
    print("\nBuilding features (per pair) ...")
    feat_dfs: dict[str, pd.DataFrame] = {}
    for pair in pairs:
        pip = _pip_size(pair)
        pair_tp = args.tp_pip * pip
        pair_sl = args.sl_pip * pip
        feat = _build_pair_features(mid_dfs[pair], args.horizon, pair_tp, pair_sl)
        feat_dfs[pair] = feat
    ...
    # Feature columns (same set as v2: 32 features)
    ...
    _print_comparison(agg, args.spread_pip)
