"""compare_multipair_v5_bidask.py — Phase 9.12/B-2 bid/ask-aware labels.

Successor to v4 (`compare_multipair_v4_atr.py`). Same vectorised eval
loop and ATR-scaled barriers, but the triple-barrier label is now
computed against bid/ask OHLC so the spread is *embedded* in the label:

    Long label:
      Entry  = ask_o[t+1]
      TP fired iff   bid_h[t+k] >= entry + tp_mult * ATR(14)
      SL fired iff   bid_l[t+k] <= entry - sl_mult * ATR(14)

    Short label:
      Entry  = bid_o[t+1]
      TP fired iff   ask_l[t+k] <= entry - tp_mult * ATR(14)
      SL fired iff   ask_h[t+k] >= entry + sl_mult * ATR(14)

Both directions are checked and the per-bar label says which direction
(if any) cleared its TP first. Bars where both directions hit TP — rare
— take the earlier of the two; bars where neither hits TP fall to 0.

Why this matters vs v4
----------------------
v4's labels were mid-based: ``mid >= mid_entry + TP`` for long. The mid
ignores the spread cost entirely, so the model trains on signals that
include trades the trader cannot actually capture (mid moved up but the
bid never reached entry_ask + TP). Net PnL in v4 was computed by
deducting a fixed spread_pip after the fact, which double-counts the
miss: the label was already optimistic.

v5's labels say "this trade actually fills at TP" or "this trade
actually fills at SL". The per-trade gross PnL in pips is therefore
the realisable PnL at the bid/ask boundary — and the only further cost
is slippage beyond bid/ask, which we model as an additional
``--slippage-pip`` (default 0.0).

Comparison with v4
------------------
At the same (TP_mult, SL_mult), v5 should:
  - Have lower hit rate than v4 (some apparent wins won't fill)
  - Have higher net Sharpe than v4 because the labels and the cost
    model now agree
  - Not need the --spread-pip subtraction (spread is in the label)

Usage
-----
    python scripts/compare_multipair_v5_bidask.py \
...
# always kept out of model features via the feature_cols exclusion set.
...
def _print_verdict(agg: dict[str, dict], spread_pip: float) -> None:
    _hdr(
        f"PHASE 9.12/B-2 VERDICT (slippage={spread_pip:.2f}pip; bid/ask spread embedded in labels)"
    )
