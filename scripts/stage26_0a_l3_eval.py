"""Stage 26.0a-β L-3 EV regression eval (rev1; spread-aware EV target).
...
ARTIFACT_ROOT = REPO_ROOT / "artifacts" / "stage26_0a"
...
# D-4: spread treatment sweep knob
SPREAD_FACTORS = {"entry_only": 1, "round_trip": 2}
...
# H3 reference (Phase 25 F1 best realised Sharpe per #284)
...
def build_l3_labels_for_dataframe(
    df: pd.DataFrame,
    pair_runtime_map: dict[str, dict],
    spread_factor: int,
    target_scale: str,
) -> pd.Series:
    """Vectorised L-3 label construction.

    Per 26.0a-α §3.1 binding:
      base_pnl    = mid-to-mid PnL (deterministic for TP/SL; M1 lookup for TIME)
      label_pre   = base_pnl - spread_at_signal_pip * spread_factor  (D-4)
    ...
    spread_pip = df["spread_at_signal_pip"].to_numpy(dtype=np.float64)
    ...
    # D-4: spread cost subtracted exactly once
    label_pre = base_pnl - spread_pip * spread_factor
...
def build_cells(include_lightgbm: bool = True) -> list[dict]:
    ...
    for spread_key in SPREAD_FACTORS:
...
def evaluate_cell(
    ...
    """
    spread_factor = SPREAD_FACTORS[cell["spread"]]
    scale = cell["scale"]
    clip_mode = cell["clip"]
    model_name = cell["model"]

    # Label construction
    train_y = build_l3_labels_for_dataframe(train_df, pair_runtime_map, spread_factor, scale)
    val_y = build_l3_labels_for_dataframe(val_df, pair_runtime_map, spread_factor, scale)
    test_y = build_l3_labels_for_dataframe(test_df, pair_runtime_map, spread_factor, scale)
    ...
    # Per-pair / per-direction trade count for the val-selected (cell, q)
    test_df_clean = test_df.loc[test_mask].reset_index(drop=True)
    traded_mask_test = pred_test >= best_q_record["cutoff"]
    valid_pnl_mask = np.isfinite(pnl_test_clean)
    in_trade = traded_mask_test & valid_pnl_mask
...
def _cell_signature(cell: dict) -> str:
    return (
        f"spread={cell['spread']:11} scale={cell['scale']:14} "
        f"clip={cell['clip']:7} model={cell['model']:16}"
    )
...
def write_eval_report(
    ...
    lines.append("")
    lines.append(
        "L-3 = spread-aware EV-regression specialisation. Rev1: quantile-of-val "
        "threshold family is the PRIMARY verdict basis; negative absolute thresholds "
        "are SECONDARY DIAGNOSTIC."
    )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "| spread | scale | clip | model | q% | cutoff | val_sharpe | val_ann_pnl | "
    ...
    for c in sorted_cells:
        ...
        lines.append(
            f"| {cell['spread']} | {cell['scale']} | {cell['clip']} | "
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"=== Stage 26.0a-beta L-3 EV regression eval (rev1; {len(args.pairs)} pairs) ===")
    print(
    ...
    for i, cell in enumerate(cells):
        ...
        print(
            ...
            f"sp={cell['spread']:11} sc={cell['scale']:14} "
            f"cl={cell['clip']:7} m={cell['model']:16} | "
