"""compare_multipair_v25_filter.py - Phase 9.X-L time-of-day + spread/ATR filter.

Cloned from v24 (calendar + realism). Adds two SUBTRACTIVE filters
applied post-SELECTOR:

  F1 time-of-day (UTC hour exclusion):  --enable-time-filter
                                        --exclude-hours-utc "3,4,21,22,23"
  F2 ATR-percentile (high-vol exclusion): --enable-spread-filter
                                          --max-atr-percentile 95

Both default OFF; without them, v25 reproduces v24 exactly.

Reviewer Rank-1 lever per docs/design/sharpe_improvement_brief.md.

Original v24 docstring follows.

compare_multipair_v24_calendar.py - Phase 9.X-H/H-2 calendar features.

Cloned from v23 (realism pack). Adds the "calendar" feature group:
9 per-(pair, bar) features derived from a curated economic calendar
(FOMC, ECB, BOJ, BOE, NFP, CPI, etc.) loaded via CalendarService.

  cal_h_to_next_base / cal_h_since_last_base
  cal_h_to_next_quote / cal_h_since_last_quote
  cal_in_pre_event (≤30min before)
  cal_in_post_event (≤60min after)
  cal_in_quiet (no event in ±2h)
  cal_imp_next / cal_imp_recent (0/0.5/1 numeric)

Causal: only event timestamps are used (public schedule). Actual /
forecast / surprise values are NOT used.

Without --feature-groups calendar, v24 reproduces v23 exactly.
Production side will use the same CalendarService loader —
backtest CSV at training time, OANDA Labs API at runtime (Phase 7
production wire-up).

Original v23 docstring follows.

compare_multipair_v23_realism.py - Phase 9.X-J Realism Pack.

Cloned from v22 (risk-sizing). Adds three production-side guards to
the backtest, each independently CLI-toggleable:

  J-1  --enable-compounding         account balance grows with profit
  J-2  --enable-risk-manager        4-constraint portfolio gate
       --max-concurrent / --max-per-instrument-pct / --max-same-direction-pct
       / --max-total-risk-pct
  J-3  --enable-csi-filter          MetaDecider Rule F5 (currency strength)
       --csi-z-threshold

Default: all OFF (reproduces v22 numbers exactly when --enable-risk-sizing
is also off).

The realism pack closes the gap between backtest Sharpe and live Sharpe.
J-1 also acts as a real return-rate lever — compounding turns 4%/month
flat into 60%/year compound (60% lift) en route to the 10%/month target.
...
from fx_ai_trading.services.calendar_service import CalendarService
...
def _compute_size_units(
    ...
    """Phase 9.X-I/I-1 risk-based sizing. Phase 9.X-J/J-1 fix: max_size_units cap."""
    if (
        sl_pip <= 0
        or risk_pct <= 0
        or balance_jpy <= 0
...
# Phase 9.X-H/H-2: economic-calendar feature columns.
...
_CALENDAR_IMP_MAP: dict[str, float] = {"low": 0.0, "medium": 0.5, "high": 1.0}
_CALENDAR_NS_PER_H = 3600.0 * 1_000_000_000.0
_CALENDAR_SENTINEL_H = 999.0
...
def _add_calendar_features(
    ...
    """Phase 9.X-H/H-2: vectorised per-bar calendar features.

    Uses np.searchsorted on per-currency sorted event timestamps so cost
    is O(n_bars × log(n_events)) per pair, vs O(n_bars × n_events) for a
    naive scan.
    """
    df = df.copy()
    parts = instrument.split("_")
    if len(parts) != 2:
        # Fall back to zero features for unknown pair format.
        for col in _CALENDAR_COLS:
            df[col] = _CALENDAR_SENTINEL_H if col.startswith("cal_h_") else 0.0
        return df
    base, quote = parts
    ...
    def _next_and_last(currency: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        events = calendar_service._events_by_currency.get(currency, [])
        if not events:
            sentinel = np.full(n, _CALENDAR_SENTINEL_H, dtype=np.float64)
            zeros = np.zeros(n, dtype=np.float64)
            return sentinel, zeros, sentinel, zeros
        ev_ns = np.array([pd.Timestamp(e.timestamp).value for e in events], dtype=np.int64)
        ev_imp = np.array([_CALENDAR_IMP_MAP[e.importance] for e in events], dtype=np.float64)

        # NEXT: smallest event_ts strictly > bar_ts.
        idx_next = np.searchsorted(ev_ns, bar_ts, side="right")
        valid_next = idx_next < len(ev_ns)
        idx_next_safe = np.where(valid_next, idx_next, 0)
        delta_next_ns = ev_ns[idx_next_safe] - bar_ts
        h_next = np.where(
            valid_next,
            delta_next_ns / _CALENDAR_NS_PER_H,
            _CALENDAR_SENTINEL_H,
        )
        imp_next = np.where(valid_next, ev_imp[idx_next_safe], 0.0)

        # LAST: largest event_ts <= bar_ts (so use side="right" minus 1).
        idx_last = np.searchsorted(ev_ns, bar_ts, side="right") - 1
        valid_last = idx_last >= 0
        idx_last_safe = np.where(valid_last, idx_last, 0)
        delta_last_ns = bar_ts - ev_ns[idx_last_safe]
        h_last = np.where(
            valid_last,
            delta_last_ns / _CALENDAR_NS_PER_H,
            _CALENDAR_SENTINEL_H,
        )
        imp_last = np.where(valid_last, ev_imp[idx_last_safe], 0.0)
        return h_next, imp_next, h_last, imp_last

    h_next_base, imp_next_base, h_last_base, imp_recent_base = _next_and_last(base)
    h_next_quote, imp_next_quote, h_last_quote, imp_recent_quote = _next_and_last(quote)

    h_to_next = np.minimum(h_next_base, h_next_quote)
    h_since_last = np.minimum(h_last_base, h_last_quote)
    imp_next = np.maximum(imp_next_base, imp_next_quote)
    imp_recent = np.maximum(imp_recent_base, imp_recent_quote)

    df["cal_h_to_next_base"] = h_next_base
    df["cal_h_since_last_base"] = h_last_base
    df["cal_h_to_next_quote"] = h_next_quote
    df["cal_h_since_last_quote"] = h_last_quote
    df["cal_in_pre_event"] = (h_to_next <= 0.5).astype(np.float64)
    df["cal_in_post_event"] = (h_since_last <= 1.0).astype(np.float64)
    df["cal_in_quiet"] = ((h_to_next > 2.0) & (h_since_last > 2.0)).astype(np.float64)
    df["cal_imp_next"] = imp_next
    df["cal_imp_recent"] = imp_recent
    return df
...
def _build_pair_features(
    ...
    # Phase 9.X-H/H-2 calendar features.
    if "calendar" in enable_groups and calendar_service is not None:
        df = _add_calendar_features(df, instrument, calendar_service)
...
def _eval_fold(
    ...
    enable_csi_filter: bool = False,
    csi_z_threshold: float = 0.5,
    enable_time_filter: bool = False,
    excluded_hours_utc: frozenset[int] = frozenset(),
    ...
    # ----- Phase 9.X-J/J-3: CSI Rule F5 post-filter -----
    # Compute per-bar CurrencyStrengthIndex from 1-bar pair returns,
    # z-scored across currencies, then reject any pick whose
    # |CSI(base) - CSI(quote)| < csi_z_threshold (matches production
    # MetaDecider Rule F5 with min_csi_diff). Trades that fail are set
    # to -1 (no trade).
    if enable_csi_filter and k_use >= 1:
        n_pairs_csi = len(all_pairs)
        pair_idx_csi: dict[str, int] = {p: i for i, p in enumerate(all_pairs)}
        # 1-bar pct returns from last_close.
        ret_mat_1 = np.zeros((n_lab, n_pairs_csi), dtype=np.float64)
        for i_p, p in enumerate(all_pairs):
            lc = pair_arrays[p]["last_close"]
            with np.errstate(divide="ignore", invalid="ignore"):
                prev = np.where(lc[:-1] > 0, lc[:-1], np.nan)
                cur = np.where(lc[1:] > 0, lc[1:], np.nan)
                ret_mat_1[1:, i_p] = (cur - prev) / np.maximum(prev, 1e-9)
        ret_mat_1 = np.nan_to_num(ret_mat_1, nan=0.0, posinf=0.0, neginf=0.0)

        # Currency mapping.
        currencies_csi = sorted({c for p in all_pairs for c in p.split("_")})
        ccy_idx_csi: dict[str, int] = {c: i for i, c in enumerate(currencies_csi)}
        n_ccy = len(currencies_csi)
        # Membership masks: which pairs have currency as base / quote.
        base_mask = np.zeros((n_ccy, n_pairs_csi), dtype=np.float64)
        quote_mask = np.zeros((n_ccy, n_pairs_csi), dtype=np.float64)
        for i_p, p in enumerate(all_pairs):
            b, q = p.split("_")
            base_mask[ccy_idx_csi[b], i_p] = 1.0
            quote_mask[ccy_idx_csi[q], i_p] = 1.0
        base_counts = base_mask.sum(axis=1, keepdims=False)
        quote_counts = quote_mask.sum(axis=1, keepdims=False)
        # Per-bar avg returns for currencies as base / quote: (n_lab, n_ccy).
        base_avg = (ret_mat_1 @ base_mask.T) / np.maximum(base_counts, 1.0)
        quote_avg = (ret_mat_1 @ quote_mask.T) / np.maximum(quote_counts, 1.0)
        raw_strength = base_avg - quote_avg  # (n_lab, n_ccy)
        # Z-score across currencies per bar.
        mu = raw_strength.mean(axis=1, keepdims=True)
        sd = raw_strength.std(axis=1, keepdims=True)
        sd_safe = np.where(sd > 1e-12, sd, 1.0)
        csi_z = (raw_strength - mu) / sd_safe  # (n_lab, n_ccy)
        # Per-pair CSI divergence: |z(base) - z(quote)|.
        pair_csi_diff = np.zeros((n_pairs_csi, n_lab), dtype=np.float64)
        for i_p, p in enumerate(all_pairs):
            b, q = p.split("_")
            pair_csi_diff[i_p] = np.abs(csi_z[:, ccy_idx_csi[b]] - csi_z[:, ccy_idx_csi[q]])

        # Map candidates → pair index (one row per candidate).
        cand_to_pair_idx = np.asarray([pair_idx_csi[p] for p, _s in candidates], dtype=np.int64)
        csi_diff_per_cand = pair_csi_diff[cand_to_pair_idx, :]  # (n_cands, n_lab)
        for r in range(k_use):
            cidx_r = sel_cand_idx_per_rank[r]
            valid = cidx_r >= 0
            if not valid.any():
                continue
            csi_for_pick = np.full(n_lab, np.inf)
            csi_for_pick[valid] = csi_diff_per_cand[cidx_r[valid], np.arange(n_lab)[valid]]
            reject = (csi_for_pick < csi_z_threshold) & valid
            sel_cand_idx_per_rank[r, reject] = -1

    # ----- Phase 9.X-J/J-2: RiskManager 4-constraint post-filter -----
    # Mirror the production RiskManagerService.allow_trade gates per-bar:
    #   C2 per-instrument: same pair cannot appear twice in one bar (auto-
    #       enforced for lgbm_only cell; explicit drop for multi-strategy)
    #   C3 per-direction:  count of long-or-short picks ≤ ceil(max_pct × K)
    #   C4 total risk:     SUM(risk_pct) ≤ max_total_risk_pct
    # C1 concurrent is implicitly capped by k_use (max K picks per bar).
    # Picks that fail any gate are set to -1 (no trade).
    if enable_risk_manager and k_use >= 1:
        # Build (n_cands, n_lab) sign matrix from pair_strat_sig.
        sign_mat = np.stack([pair_strat_sig[c] for c in candidates], axis=0).astype(np.int8)
        for b in range(n_lab):
            picks_long = 0
            picks_short = 0
            picks_total = 0
            ...
            cumulative_risk = 0.0
            for r in range(k_use):
                cidx = int(sel_cand_idx_per_rank[r, b])
                if cidx < 0:
                    continue
                pair, _s = candidates[cidx]
                sig = int(sign_mat[cidx, b])
                # C2 per-instrument
    ...
    # ----- Phase 9.X-L F1: Time-of-day exclusion -----
    # Reject picks whose bar timestamp falls in an excluded UTC hour.
    # Cheap, structural, ~constant ~5-15% trade reduction depending on hours.
    if enable_time_filter and excluded_hours_utc and k_use >= 1:
        bar_hours = pd.to_datetime(base_ts).hour  # int array shape (n_lab,)
        excluded_mask = np.isin(bar_hours, np.asarray(list(excluded_hours_utc)))
        for r in range(k_use):
            sel_cand_idx_per_rank[r, excluded_mask] = -1

    # ----- Phase 9.X-L F2: ATR-percentile (high-vol) exclusion -----
    # Reject picks where pair's ATR exceeds its per-pair quantile threshold
    # (proxy for spread blow-out / illiquid bars). Requires per-pair ATR
    # already in pair_arrays.
    if enable_spread_filter and k_use >= 1:
        # Pre-compute per-pair ATR threshold (in absolute units, not pip).
        pair_atr_threshold: dict[str, float] = {}
        q = max(0.0, min(100.0, max_atr_percentile)) / 100.0
        for pair in all_pairs:
            atr_arr = pair_arrays[pair]["atr"]
            atr_finite = atr_arr[np.isfinite(atr_arr) & (atr_arr > 0)]
            if atr_finite.size > 0:
                pair_atr_threshold[pair] = float(np.quantile(atr_finite, q))
            else:
                pair_atr_threshold[pair] = float("inf")
        # Build per-(candidate, bar) high-ATR mask.
        cand_pair_for_idx = [p for p, _s in candidates]
        atr_mat = np.stack(
            [pair_arrays[p]["atr"] for p in cand_pair_for_idx], axis=0
        )  # (n_cands, n_lab)
        thresh_arr = np.asarray(
            [pair_atr_threshold.get(p, float("inf")) for p in cand_pair_for_idx]
        ).reshape(-1, 1)
        too_high = atr_mat > thresh_arr  # (n_cands, n_lab)
        for r in range(k_use):
            cidx_r = sel_cand_idx_per_rank[r]
            valid = cidx_r >= 0
            if not valid.any():
                continue
            high_for_pick = np.zeros(n_lab, dtype=bool)
            high_for_pick[valid] = too_high[cidx_r[valid], np.arange(n_lab)[valid]]
            sel_cand_idx_per_rank[r, high_for_pick] = -1
    ...
    if enable_risk_sizing and sel_net_jpy is not None and sel_net_jpy.size > 0:
        ...
        # Phase 9.X-K: per-bar JPY series (full n_lab including zero bars)
        # plus aligned timestamps in ns for daily-Sharpe aggregation.
        # All trades resolve within their bar (TB labelling), so per-bar
        # JPY is realized PnL = MTM closure for that bar's positions.
...
def _aggregate(fold_results: list[dict], initial_balance_jpy: float = 300_000.0) -> dict:
    ...
    for s in strats:
        ...
        # Phase 9.X-K: daily-bar JPY aggregation → daily Sharpe (annualized) + DD on equity.
...
def _compute_daily_metrics(
    ...
    Concatenate per-bar JPY PnL across folds, group by calendar day (UTC),
    fill missing days with 0 (no-trade days), then build equity curve from
    initial_balance and compute annualized Sharpe and max drawdown on that
    equity series.

    Empty inputs return zero metrics. Daily Sharpe annualised by sqrt(252).
    """
    bar_pnls: list[float] = []
    bar_ts_ns: list[int] = []
    for fr in fold_results:
    ...
    # Group by UTC calendar day. normalize() drops time component.
    ...
    # Reindex to full date range so no-trade days are 0 returns.
    ...
    # Build equity curve. Start from initial_balance the day BEFORE the first
    # series day so that the first daily return is well defined.
    ...
    # Max DD on equity curve (post-initial point onwards): worst peak-to-trough.
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Phase 9.X-J/J-1: account balance compounds across folds. Each "
            "fold's JPY PnL is added to running balance, and per-trade size "
            "for the next fold uses the updated balance. Requires "
            "--enable-risk-sizing. Default off (constant balance reproduces "
            "v22 behaviour)."
        ),
    )
    parser.add_argument(
        ...
        default=False,
        help=(
            "Phase 9.X-J/J-2: apply 4-constraint portfolio gate post-SELECTOR. "
            "Mirrors RiskManagerService.allow_trade. C1 concurrent is "
            "implicit (≤ K). C2 per-instrument: no pair twice per bar. "
            "C3 per-direction: ≤ max-same-direction-pct of accepted picks. "
            "C4 total risk: SUM(risk_pct) ≤ max-total-risk-pct."
        ),
    )
    parser.add_argument(
        "--max-per-instrument-pct",
        type=float,
        default=50.0,
        help="Phase 9.X-J/J-2: per-instrument exposure cap (currently advisory).",
    )
    parser.add_argument(
        "--max-same-direction-pct",
        type=float,
        default=70.0,
        help="Phase 9.X-J/J-2: same-direction (long/short) cap as %% of picks.",
    )
    parser.add_argument(
        "--max-total-risk-pct",
        type=float,
        default=5.0,
        help="Phase 9.X-J/J-2: cap on SUM(risk_pct) across concurrent picks.",
    )
    parser.add_argument(
        "--enable-csi-filter",
        action="store_true",
        default=False,
        help=(
            "Phase 9.X-J/J-3: apply MetaDecider Rule F5 — reject picks "
            "whose |CSI(base) - CSI(quote)| < csi-z-threshold (z-score "
            "units). Mirrors production CurrencyStrengthIndex filter."
        ),
    )
    parser.add_argument(
        "--csi-z-threshold",
        type=float,
        default=0.5,
        help="Phase 9.X-J/J-3: minimum CSI z-score divergence. Default 0.5.",
    )
    parser.add_argument(
        "--calendar-path",
        type=str,
        default="data/economic_calendar/events_2025_2026.csv",
        help=(
            "Phase 9.X-H/H-2: path to curated economic calendar CSV. "
            "Required when --feature-groups includes 'calendar'."
        ),
    )
    parser.add_argument(
        "--enable-time-filter",
        ...
        default=False,
        help=(
            "Phase 9.X-L F1: reject picks whose bar's UTC hour is in "
            "--exclude-hours-utc. Default off."
        ),
    )
    parser.add_argument(
        "--exclude-hours-utc",
        type=str,
        default="3,4,21,22,23",
        help=(
            "Phase 9.X-L F1: comma-separated UTC hours to exclude. "
            "Default '3,4,21,22,23' (Tokyo lunch, NY close, Asia handover)."
        ),
    )
    parser.add_argument(
        ...
        default=False,
        help=(
            "Phase 9.X-L F2: reject picks whose pair ATR exceeds "
            "--max-atr-percentile of that pair's ATR distribution."
        ),
    )
    parser.add_argument(
        "--max-atr-percentile",
        type=float,
        default=95.0,
        help="Phase 9.X-L F2: per-pair ATR percentile threshold. Default 95.",
    )
    args = parser.parse_args(argv)
    if args.enable_compounding and not args.enable_risk_sizing:
        print(
            "ERROR: --enable-compounding requires --enable-risk-sizing "
            "(compounding is meaningless without ¥-based PnL).",
            file=sys.stderr,
        )
        return 2
    ...
    # Phase 9.X-L F1: parse --exclude-hours-utc into a frozenset[int].
    try:
        excluded_hours_utc: frozenset[int] = frozenset(
            int(h.strip()) for h in args.exclude_hours_utc.split(",") if h.strip()
        )
    except ValueError:
        print(
            f"ERROR: --exclude-hours-utc values must be integers (got {args.exclude_hours_utc!r})",
            file=sys.stderr,
        )
        return 2
    for h in excluded_hours_utc:
        if not (0 <= h <= 23):
            print(f"ERROR: --exclude-hours-utc values must be in 0-23 (got {h})", file=sys.stderr)
            return 2

    enable_groups = frozenset(g.strip() for g in args.feature_groups.split(",") if g.strip())
    valid_group_names = set(_PHASE_X_B_GROUPS.keys()) | {"calendar"}
    invalid_groups = enable_groups - valid_group_names
    if invalid_groups:
        print(
            f"ERROR: invalid feature group(s): {sorted(invalid_groups)} "
            f"(valid: {sorted(valid_group_names)})",
            file=sys.stderr,
        )
        return 2

    # Phase 9.X-H/H-2: load calendar service if calendar group enabled.
    calendar_service: CalendarService | None = None
    if "calendar" in enable_groups:
        cal_path = Path(args.calendar_path)
        if not cal_path.exists():
            print(f"ERROR: --calendar-path not found: {cal_path}", file=sys.stderr)
            return 2
        calendar_service = CalendarService.from_csv(cal_path)
        n_events = sum(
            calendar_service.event_count(c)
            for c in ("USD", "EUR", "JPY", "GBP", "AUD", "CAD", "CHF", "NZD")
        )
        print(f"Calendar: loaded {n_events} events from {cal_path}")
    ...
    # Phase 9.X-J/J-1: per-(cell, K) running balance for compounding.
    # Without --enable-compounding, this stays at args.initial_balance_jpy
    # the whole run (reproduces v22 behaviour).
    running_balance_by_key: dict[tuple[str, int], float] = {
        key: args.initial_balance_jpy for key in sweep_keys
    }
    balance_history_by_key: dict[tuple[str, int], list[float]] = {
        key: [args.initial_balance_jpy] for key in sweep_keys
    }
    ...
    for fid, fold in enumerate(folds):
        ...
        for cell_name, k in sweep_keys:
            ...
            key = (cell_name, k)
            # Phase 9.X-J/J-1: pass running balance instead of initial.
            balance_for_fold = running_balance_by_key[key]
            results, pair_counts, strat_counts, pnl_series, per_pick = _eval_fold(
                ...
                enable_csi_filter=args.enable_csi_filter,
                csi_z_threshold=args.csi_z_threshold,
                enable_time_filter=args.enable_time_filter,
                excluded_hours_utc=excluded_hours_utc,
                enable_spread_filter=args.enable_spread_filter,
                max_atr_percentile=args.max_atr_percentile,
            )
            fold_results_by_key[key].append(results)
            # Phase 9.X-J/J-1: update running balance from this fold's ¥ PnL.
            if args.enable_compounding and args.enable_risk_sizing:
                fold_pnl_jpy = float(results["SELECTOR"].get("net_pnl_jpy", 0.0))
                # Floor at 1 to avoid negative-balance / divide-by-zero downstream.
                running_balance_by_key[key] = max(1.0, balance_for_fold + fold_pnl_jpy)
            balance_history_by_key[key].append(running_balance_by_key[key])
    ...
    if len(cell_summaries) > 1:
        ...
        if args.enable_compounding:
            ...
            for cell_name, k, _agg, _rho in cell_summaries:
                key = (cell_name, k)
                final_balance = running_balance_by_key[key]
                ret = (final_balance - args.initial_balance_jpy) / args.initial_balance_jpy
                if years > 0:
                    annualized = (final_balance / args.initial_balance_jpy) ** (1.0 / years) - 1.0
                else:
                    annualized = 0.0
                print(
        ...
        # Phase 9.X-L: time-of-day + spread filter activity summary.
        if args.enable_time_filter or args.enable_spread_filter:
            _hdr(
                "PHASE 9.X-L - SUBTRACTIVE FILTER ACTIVITY "
                f"(time={'on' if args.enable_time_filter else 'off'}, "
                f"spread={'on' if args.enable_spread_filter else 'off'})"
            )
            print(f"  Excluded UTC hours: {sorted(excluded_hours_utc) or 'none'}")
            print(f"  ATR percentile cap: {args.max_atr_percentile}%")
            print("  See pip/JPY/Daily summary blocks below for filtered-trade results.")
            print()

        # Phase 9.X-K: daily annualized Sharpe + max DD on equity curve.
        if args.enable_risk_sizing:
            _hdr(
                f"PHASE 9.X-K - DAILY ANNUALIZED SHARPE + MAX DD "
